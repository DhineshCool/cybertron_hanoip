		...
		struct sde_encoder_phys *phys_enc)
{
	...
	if (intf_cfg->intf_count >= MAX_INTF_PER_CTL_V1) {
		pr_debug_once("invalid inf_count %d\n", intf_cfg->intf_count);
		return;
	}
	...
	for (i = 0; i < sde_enc->num_phys_encs; i++) {
		...
		if (active) {
			if (cfg.dsc_count >= MAX_DSC_PER_CTL_V1) {
				pr_debug_once("Invalid dsc count:%d\n",
						cfg.dsc_count);
				return -EINVAL;
			}
...
static int _sde_encoder_switch_to_watchdog_vsync(struct drm_encoder *drm_enc)
{
	...
	if (!drm_enc) {
		pr_debug_once("invalid drm encoder\n");
		return -EINVAL;
	}
	...
	struct input_dev *dev, const struct input_device_id *id)
{
	...
	if (rc) {
		pr_debug_once("failed to register input handle\n");
		goto error;
	}

	rc = input_open_device(handle);
	if (rc) {
		pr_debug_once("failed to open input device\n");
		...
		struct input_handler *input_handler)
{
	...
	if (rc) {
		pr_debug_once("input_register_handler failed, rc= %d\n", rc);
		kfree(input_handler);
		return rc;
	}
