...
static int msm_drm_init(struct device *dev, struct drm_driver *drv)
{
	...
	if (ret) {
		pr_debug_once("sde power resource init failed\n");
		goto power_init_fail;
	}

	priv->pclient = sde_power_client_create(&priv->phandle, "sde");
	if (IS_ERR_OR_NULL(priv->pclient)) {
		pr_debug_once("sde power client create failed\n");
		ret = -EINVAL;
	...
	if (kms && kms->funcs && kms->funcs->postinit) {
		...
		if (ret) {
			pr_debug_once("kms post init failed: %d\n", ret);
			goto fail;
		}
	}
...
fail:
	if (rc == -EDEADLK) {
		...
		pr_debug_once("last close failed: %d\n", rc);
	}
			...
			struct drm_file *file_priv)
{
	...
	if (power_ctrl->enable) {
		...
		pr_debug_once("ignoring, unbalanced disable\n");
	}
