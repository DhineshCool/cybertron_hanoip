...
int sde_rsc_mode2_exit(struct sde_rsc_priv *rsc, enum sde_rsc_state state)
{
	...
	if (rc)
		pr_debug_once("vdd reg is not enabled yet\n");
...
static int sde_rsc_mode2_entry(struct sde_rsc_priv *rsc)
{
	...
	if (rsc->sw_fs_enabled) {
		...
		if (rc) {
			pr_debug_once("vdd reg fast mode set failed rc:%d\n", rc);
			return rc;
		}
	}
	...
	for (i = 0; i <= MAX_MODE2_ENTRY_TRY; i++) {
		...
		reg = dss_reg_r(&rsc->drv_io,
				SDE_RSCC_SEQ_PROGRAM_COUNTER, rsc->debug_mode);
		pr_debug_once("mdss gdsc power down failed, instruction:0x%x, rc:%d\n",
				reg, rc);
						...
						enum sde_rsc_state state)
{
	...
	if (rsc->power_collapse) {
		...
		if (rc)
			pr_debug_once("power collapse: mode2 exit failed\n");
		else
	...
	case SDE_RSC_IDLE_STATE:
		...
		if (rc)
			pr_debug_once("power collapse - mode 2 entry failed\n");
		else
			rsc->power_collapse = true;
		break;

	default:
		pr_debug_once("state:%d handling is not supported\n", state);
		break;
	}
...
int rsc_hw_init(struct sde_rsc_priv *rsc)
{
	...
	if (rc) {
		pr_debug_once("rsc hw qtimer init failed\n");
		goto end;
	}

	rc = rsc_hw_wrapper_init(rsc);
	if (rc) {
		pr_debug_once("rsc hw wrapper init failed\n");
		goto end;
	}
	...
	if (rc) {
		pr_debug_once("rsc sequencer memory init failed\n");
		goto end;
	}

	rc = rsc_hw_solver_init(rsc);
	if (rc) {
		pr_debug_once("rsc solver init failed\n");
		goto end;
	}

	rc = rsc_hw_pdc_init(rsc);
	if (rc) {
		pr_debug_once("rsc hw pdc init failed\n");
		goto end;
	}
