					...
					bool enable)
{
	...
	if (!mctrl) {
		pr_debug_once("Invalid controller\n");
		return -EINVAL;
	}
	...
	rc = dsi_ctrl_config_clk_gating(mctrl->ctrl, enable, PIXEL_CLK |
							DSI_PHY);
	if (rc) {
		pr_debug_once("[%s] failed to %s clk gating, rc=%d\n",
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to %s pixel clk gating, rc=%d\n",
				display->name, enable ? "enable" : "disable",
				rc);
			return rc;
		}
		...
		void *display, u32 bl_lvl)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to enable DSI core clocks, rc=%d\n",
		       dsi_display->name, rc);
		goto error;
	}
	...
	if (rc)
		pr_debug_once("unable to set backlight\n");

	rc = dsi_display_clk_ctrl(dsi_display->dsi_clk_handle,
			DSI_CORE_CLK, DSI_CLK_OFF);
	if (rc) {
		pr_debug_once("[%s] failed to disable DSI core clocks, rc=%d\n",
		       dsi_display->name, rc);
		goto error;
	}
...
int dsi_display_set_param(void *display, struct msm_param_info *param_info)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to panel to set param. rc=%d\n",
				dsi_display->name, rc);
	return rc;
}
...
static int dsi_display_cmd_engine_enable(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to enable cmd engine, rc=%d\n",
		       display->name, rc);
		goto done;
	}
...
static int dsi_display_cmd_engine_disable(struct dsi_display *display)
{
	...
	if (display->cmd_engine_refcount == 0) {
		pr_debug_once("[%s] Invalid refcount\n", display->name);
		goto done;
...
static void dsi_display_aspace_cb_locked(void *cb_data, bool is_detach)
{
	...
	if (!cb_data) {
		pr_debug_once("aspace cb called with invalid cb_data\n");
		return;
	}
	...
	if (is_detach) {
		...
		if (rc) {
			pr_debug_once("failed to get the iova rc %d\n", rc);
			goto end;
		}
		...
		if (IS_ERR_OR_NULL(display->vaddr)) {
			pr_debug_once("failed to get va rc %d\n", rc);
			goto end;
		}
	}
...
static void dsi_display_register_te_irq(struct dsi_display *display)
{
	...
	rc = devm_request_irq(dev, te_irq, dsi_display_panel_te_irq_handler,
			      ...
			      "TE_GPIO", display);
	if (rc) {
		pr_debug_once("TE request_irq failed for ESD rc:%d\n", rc);
...
static int dsi_host_alloc_cmd_tx_buffer(struct dsi_display *display)
{
	...
	if ((display->tx_cmd_buf) == NULL) {
		pr_debug_once("Failed to allocate cmd tx buf memory\n");
		rc = -ENOMEM;
		goto error;
	}
	...
	if (!display->aspace) {
		pr_debug_once("failed to get aspace\n");
		rc = -EINVAL;
		goto free_gem;
	}
	/* register to aspace */
	...
	if (rc) {
		pr_debug_once("failed to register callback %d", rc);
		goto free_gem;
	}
...
static bool dsi_display_validate_reg_read(struct dsi_panel *panel)
{
	...
	for (i = 0; i < len; ++i) {
		pr_debug_once("%s: read back[%i] = 0x%x and expected[%d] =0x%x\n",
...
static void dsi_display_parse_te_data(struct dsi_display *display)
{
	...
	pdev = display->pdev;
	if (!pdev) {
		pr_debug_once("Inavlid platform device\n");
		return;
	}
	...
	if (rc || (val  > MAX_TE_SOURCE_ID)) {
		pr_debug_once("invalid vsync source selection\n");
		val = 0;
	}
		...
		struct dsi_panel *panel)
{
	...
	for (i = 0; i < count; ++i) {
		...
		if (rc <= 0) {
			pr_debug_once("rx cmd transfer failed rc=%d\n", rc);
			return rc;
		}
...
static int dsi_display_status_reg_read(struct dsi_display *display)
{
	...
	if (display->tx_cmd_buf == NULL) {
		...
		if (rc) {
			pr_debug_once("failed to allocate cmd tx buffer memory\n");
			goto done;
		}
	}
	...
	if (rc) {
		pr_debug_once("cmd engine enable failed\n");
		return -EPERM;
	}

	rc = dsi_display_validate_status(m_ctrl, display->panel);
	if (rc <= 0) {
		pr_debug_once("[%s] read status failed on master,rc=%d\n",
		       display->name, rc);
		goto exit;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc <= 0) {
			pr_debug_once("[%s] read status failed on slave,rc=%d\n",
			       display->name, rc);
			goto exit;
		}
	}
...
int dsi_display_set_tearing(void *display, bool enable)
{
	...
	if (!panel->panel_initialized) {
		pr_debug_once("Panel not initialized\n");
		...
		struct dsi_cmd_desc *cmd, u8 *payload, u32 payload_len)
{
	...
	if (cmd->msg.tx_len > payload_len) {
		pr_debug_once("Incorrect payload length tx_len %zu, payload_len %d\n",
				...
				size_t tx_len, u8 *tx_data_type)
{
	...
	*tx_data_type = 0;
	...
	if (cmd_type > DISPUTIL_DSI_GENERIC) {
		pr_debug_once("%s: Invalid DSI package type[%d] = 0x%x\n",
	...
	if (dsi_cmd == DISPUTIL_DSI_WRITE) {
		...
		if ((cmd_type == DISPUTIL_DSI_DCS) && (tx_len > 1)) {
			...
			/* DCS read */
			pr_debug_once("DCS read supports no parameter. tx_len=%d\n",
					(u32)tx_len);
			rc = -EINVAL;
		} else {
			switch(tx_len) {
			...
			case 3:
				...
				break;
			default:
				pr_debug_once("Invalid for tx_len= %d\n", (u32)tx_len);
				break;
			}
		}
	} else
	...
	if (rc)
		pr_debug_once("Invalid input data: dsi_cmd=0x%x tx_data_type =0x%x for tx_len=0x%x\n",
	...
	struct motUtil *motUtil_data)
{
	...
	if ((cmd->msg.tx_len > (cmd_buf_len - DISPUTIL_PAYLOAD)) ||
		...
		pr_debug_once("Incorrect payload length tx_len:%ld, payload_len_max=%d cmd_buf_len=%d\n",
		...
		u32 cmd_buf_len)
{
	...
	if (!dsi_display || !cmd_buf) {
		pr_debug_once("[DSI] invalid params\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("[DSI] command prepare failed. rc %d\n", rc);
		return rc;
	}
	...
	if (rc || !state) {
		pr_debug_once("[DSI] Invalid host state %d rc %d\n",
				state, rc);
		rc = -EPERM;
		goto end;
	}
...
static int dsi_display_elvss_read(struct dsi_display *display)
{
	...
	if (rc <= 0) {
		pr_debug_once("[%s] read elvss failed on master,rc=%d\n",
		       display->name, rc);
		goto exit;
	}
...
int dsi_display_soft_reset(void *display)
{
	...
	display_for_each_ctrl(i, dsi_display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to soft reset host_%d, rc=%d\n",
		...
		void *display)
{
	...
	if (!dsi_display || !dsi_display->panel) {
		pr_debug_once("Invalid params(s) dsi_display %pK, panel %pK\n",
			dsi_display,
		...
		int power_mode, void *disp)
{
	...
	if (!display || !display->panel) {
		pr_debug_once("invalid display/panel\n");
		return -EINVAL;
	}
...
static bool dsi_display_is_te_based_esd(struct dsi_display *display)
{
	...
	if (!display->panel) {
		pr_debug_once("Invalid panel data\n");
		return false;
	}
				  ...
				  loff_t *ppos)
{
	...
	if (display->esd_trigger) {
		...
		if (rc) {
			pr_debug_once("Failed to trigger ESD attack\n");
			goto error;
		}
	}
				  ...
				  loff_t *ppos)
{
	...
	if (!esd_config) {
		pr_debug_once("Invalid panel esd config\n");
		rc = -EINVAL;
		goto error;
	}
	...
	if (!strcmp(buf, "reg_read\n")) {
		...
		if (rc) {
			pr_debug_once("failed to alter esd check mode,rc=%d\n",
						rc);
			rc = user_len;
			goto error;
		}
				 ...
				 loff_t *ppos)
{
	...
	if (!panel_param) {
		pr_debug_once("invalid panel_param.\n");
		return -EINVAL;
	}
...
static int dsi_display_debugfs_init(struct dsi_display *display)
{
	...
	if (IS_ERR_OR_NULL(dir)) {
		rc = PTR_ERR(dir);
		pr_debug_once("[%s] debugfs create dir failed, rc = %d\n",
		       display->name, rc);
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		...
		pr_debug_once("[%s] debugfs create dump info file failed, rc=%d\n",
		       display->name, rc);
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		rc = PTR_ERR(dump_file);
		pr_debug_once("[%s] debugfs for esd trigger file failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		rc = PTR_ERR(dump_file);
		pr_debug_once("[%s] debugfs for esd check mode failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(misr_data)) {
		...
		pr_debug_once("[%s] debugfs create misr datafile failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (IS_ERR_OR_NULL(dump_file)) {
			rc = PTR_ERR(dump_file);
			pr_debug_once("[%s] debugfs create %s failed, rc=%d\n",
	...
	if (!debugfs_create_bool("ulps_feature_enable", 0600, dir,
		...
		pr_debug_once("[%s] debugfs create ulps feature enable file failed\n",
		       display->name);
		goto error_remove_dir;
	}
	...
	if (!debugfs_create_bool("ulps_suspend_feature_enable", 0600, dir,
		...
		pr_debug_once("[%s] debugfs create ulps-suspend feature enable file failed\n",
		       display->name);
		goto error_remove_dir;
	}
	...
	if (!debugfs_create_bool("ulps_status", 0400, dir,
		...
		pr_debug_once("[%s] debugfs create ulps status file failed\n",
		       display->name);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(dump_file)) {
		rc = PTR_ERR(dump_file);
		pr_debug_once("[%s] debugfs for hbm file failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
...
static int dsi_display_set_ulps(struct dsi_display *display, bool enable)
{
	...
	if (rc == DSI_PHY_ULPS_ERROR) {
		pr_debug_once("Ulps PHY state change(%d) failed\n", enable);
		return -EINVAL;
	}
		...
		if (rc) {
			pr_debug_once("Ulps controller state change(%d) failed\n",
					enable);
			return rc;
		}
		display_for_each_ctrl(i, display) {
...
static int dsi_display_set_clamp(struct dsi_display *display, bool enable)
{
	...
	if (rc) {
		pr_debug_once("DSI ctrl clamp state change(%d) failed\n", enable);
		return rc;
	}

	rc = dsi_phy_set_clamp_state(m_ctrl->phy, enable);
	if (rc) {
		pr_debug_once("DSI phy clamp state change(%d) failed\n", enable);
		return rc;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("DSI Clamp state change(%d) failed\n", enable);
			return rc;
		}
...
static int dsi_display_ctrl_setup(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("DSI controller setup failed\n");
		return rc;
	}
...
static int dsi_display_ctrl_power_on(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] Failed to set power state, rc=%d\n",
			       ctrl->ctrl->name, rc);
			goto error;
		}
	}
...
static int dsi_display_ctrl_power_off(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		rc = dsi_ctrl_set_power_state(ctrl->ctrl,
			DSI_CTRL_POWER_VREG_OFF);
		if (rc) {
			pr_debug_once("[%s] Failed to power off, rc=%d\n",
			       ctrl->ctrl->name, rc);
			goto error;
		}
	}
					...
					unsigned int display_type)
{
	...
	if (display_type >= MAX_DSI_ACTIVE_DISPLAY) {
		pr_debug_once("display_type=%d not supported\n", display_type);
		goto end;
	}
	...
	if (kstrtol(str + strlen(":config"), INT_BASE_10,
		...
		pr_debug_once("invalid config index override: %s\n", boot_str);
		goto end;
	}
	...
	if (kstrtol(str + strlen(":timing"), INT_BASE_10,
		...
		pr_debug_once("invalid timing index override: %s. resetting both timing and config\n",
			boot_str);
...
static int dsi_display_set_clk_src(struct dsi_display *display, bool on)
{
	...
	if (display->panel->host_config.phy_type == DSI_PHY_TYPE_CPHY) {
		...
		if (rc) {
			pr_debug_once("failed update mux parent to CPHY\n");
			return rc;
		}
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to set source clocks for master, rc=%d\n",
			   display->name, rc);
		return rc;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to set source clocks, rc=%d\n",
				   display->name, rc);
			return rc;
		}
	}
		...
		bool enable)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to %s phy reset, rc=%d\n",
...
static int dsi_display_ctrl_update(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to update host_%d, rc=%d\n",
				   display->name, i, rc);
...
static int dsi_display_ctrl_init(struct dsi_display *display)
{
	...
	if (!display->panel->ulps_suspend_enabled || !display->ulps_enabled) {
		display_for_each_ctrl(i, display) {
			...
			if (rc) {
				pr_debug_once("[%s] failed to init host_%d, rc=%d\n",
				       display->name, i, rc);
				goto error_host_deinit;
			}
		}
...
static int dsi_display_ctrl_deinit(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to deinit host_%d, rc=%d\n",
			       display->name, i, rc);
		}
	}
...
static int dsi_display_ctrl_host_enable(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to enable host engine, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		rc = dsi_ctrl_set_host_engine_state(ctrl->ctrl,
						    DSI_CTRL_ENGINE_ON);
		if (rc) {
			pr_debug_once("[%s] failed to enable sl host engine, rc=%d\n",
			       display->name, rc);
			goto error_disable_master;
		}
	}
...
static int dsi_display_ctrl_host_disable(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		rc = dsi_ctrl_set_host_engine_state(ctrl->ctrl,
						    DSI_CTRL_ENGINE_OFF);
		if (rc)
			pr_debug_once("[%s] failed to disable host engine, rc=%d\n",
			       display->name, rc);
	}
...
static int dsi_display_vid_engine_enable(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to enable vid engine, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_vid_engine_disable(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		rc = dsi_ctrl_set_vid_engine_state(ctrl->ctrl,
						   DSI_CTRL_ENGINE_OFF);
		if (rc)
			pr_debug_once("[%s] failed to disable vid engine, rc=%d\n",
			       display->name, rc);
	}

	rc = dsi_ctrl_set_vid_engine_state(m_ctrl->ctrl, DSI_CTRL_ENGINE_OFF);
	if (rc)
		pr_debug_once("[%s] failed to disable mvid engine, rc=%d\n",
		       display->name, rc);
...
static int dsi_display_phy_enable(struct dsi_display *display)
{
	...
	rc = dsi_phy_enable(m_ctrl->phy,
			    ...
			    display->is_cont_splash_enabled);
	if (rc) {
		pr_debug_once("[%s] failed to enable DSI PHY, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_phy_disable(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc)
			pr_debug_once("[%s] failed to disable DSI PHY, rc=%d\n",
			       display->name, rc);
	}
				     ...
				     const struct mipi_dsi_msg *msg)
{
	...
	if (rc) {
		pr_debug_once("[%s] cmd transfer failed on master,rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] cmd transfer failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("[%s] cmd trigger failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
	...
	if (rc) {
		pr_debug_once("[%s] cmd trigger failed for master, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_phy_sw_reset(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to reset phy, rc=%d\n", display->name, rc);
		goto error;
	}
				 ...
				 const struct mipi_dsi_msg *msg)
{
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_ON);
	if (rc) {
		pr_debug_once("[%s] failed to enable all DSI clocks, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to wake up display, rc=%d\n",
		       display->name, rc);
	...
	if (display->ctrl_count > 1 && !(msg->flags & MIPI_DSI_MSG_UNICAST)) {
		...
		if (rc) {
			pr_debug_once("[%s] cmd broadcast failed, rc=%d\n",
			       display->name, rc);
			goto error_disable_cmd_engine;
		}
	} else {
...
error_disable_cmd_engine:
	...
	if (ret) {
		pr_debug_once("[%s]failed to disable DSI cmd engine, rc=%d\n",
				display->name, ret);
	}
error_disable_clks:
	ret = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_OFF);
	if (ret) {
		pr_debug_once("[%s] failed to disable all DSI clocks, rc=%d\n",
		       display->name, ret);
	}
error:
	return rc;
...
static int dsi_display_mipi_host_init(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to register mipi dsi host, rc=%d\n",
		       display->name, rc);
		goto error;
	}
...
static int dsi_display_clocks_init(struct dsi_display *display)
{
	...
	for (i = 0; i < num_clk; i++) {
		...
		if (IS_ERR_OR_NULL(dsi_clk)) {
			...
			pr_debug_once("failed to get %s, rc=%d\n", clk_name, rc);
	...
	struct dsi_clk_ctrl_info clk_state_info)
{
	...
	if (clk_state_info.client == DSI_CLK_REQ_MDP_CLIENT) {
		...
		pr_debug_once("invalid clk handle, return error\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to %d DSI %d clocks, rc=%d\n",
			   ...
			   enum dsi_clk_state new_state)
{
	...
	if ((clk & DSI_LINK_CLK) && (new_state == DSI_CLK_OFF) &&
		...
		if (rc)
			pr_debug_once("%s: failed enable ulps, rc = %d\n",
			       __func__, rc);
	}
	...
	if ((clk & DSI_LINK_CLK) && (new_state == DSI_CLK_OFF) &&
		...
		if (rc)
			pr_debug_once("[%s] failed to disable clk gating, rc=%d\n",
					display->name, rc);
	}
	...
	if ((clk & DSI_CORE_CLK) && (new_state == DSI_CLK_OFF)) {
		...
		if (dsi_panel_initialized(display->panel) ||
			...
			if (rc)
				pr_debug_once("%s: Failed to enable dsi clamps. rc=%d\n",
					__func__, rc);
			...
			if (rc)
				pr_debug_once("%s: failed to disable ulps. rc=%d\n",
					__func__, rc);
		}
			   ...
			   enum dsi_clk_state curr_state)
{
	...
	if ((clk & DSI_LINK_CLK) && (l_type & DSI_LINK_LP_CLK)) {
		...
		if (display->ulps_enabled && mmss_clamp) {
			...
			rc = dsi_display_set_ulps(display, true);
			if (rc) {
				pr_debug_once("%s: Failed to enter ULPS. rc=%d\n",
					__func__, rc);
				goto error;
			}
		}
		...
		if (rc) {
			pr_debug_once("%s: Failed to disable dsi clamps. rc=%d\n",
				__func__, rc);
			goto error;
		}
	}
	...
	if ((clk & DSI_LINK_CLK) && (l_type & DSI_LINK_HS_CLK)) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to enable clk gating %d\n",
					display->name, rc);
			goto error;
		}
	}
			    ...
			    enum dsi_clk_state curr_state)
{
	...
	if ((clk_type & DSI_CORE_CLK) &&
		...
		if (rc)
			pr_debug_once("[%s] failed to power off PHY, rc=%d\n",
				   display->name, rc);

		rc = dsi_display_ctrl_power_off(display);
		if (rc)
			pr_debug_once("[%s] failed to power DSI vregs, rc=%d\n",
				   display->name, rc);
	}
	return rc;
}
			  ...
			  enum dsi_clk_state new_state)
{
	...
	if ((clk_type & DSI_CORE_CLK) && (new_state == DSI_CLK_ON)) {
	  ...
	  pr_debug("updating power states for ctrl and phy\n");
		...
		if (rc) {
			pr_debug_once("[%s] failed to power on dsi controllers, rc=%d\n",
				   display->name, rc);
			return rc;
		}
		...
		if (rc) {
			pr_debug_once("[%s] failed to power on dsi phy, rc = %d\n",
				   display->name, rc);
			return rc;
		}
...
static int dsi_display_parse_dt(struct dsi_display *display)
{
	...
	if (!phy_count || !display->ctrl_count) {
		pr_debug_once("no ctrl/phys found\n");
		rc = -ENODEV;
		goto error;
	}

	if (phy_count != display->ctrl_count) {
		pr_debug_once("different ctrl and phy counts\n");
		rc = -ENODEV;
		goto error;
	}
	...
	if (!display->panel_of) {
		pr_debug_once("No Panel device present\n");
		rc = -ENODEV;
		goto error;
	}
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (IS_ERR_OR_NULL(ctrl->ctrl)) {
			...
			pr_debug_once("failed to get dsi controller, rc=%d\n", rc);
			ctrl->ctrl = NULL;
			goto error_ctrl_put;
		}
		...
		if (IS_ERR_OR_NULL(ctrl->phy)) {
			rc = PTR_ERR(ctrl->phy);
			pr_debug_once("failed to get phy controller, rc=%d\n", rc);
	...
	if (IS_ERR_OR_NULL(display->panel)) {
		...
		pr_debug_once("failed to get panel, rc=%d\n", rc);
		display->panel = NULL;
		goto error_ctrl_put;
	}
	...
	if (rc) {
		pr_debug_once("Lane map not found, rc=%d\n", rc);
		goto error_ctrl_put;
	}

	rc = dsi_display_clocks_init(display);
	if (rc) {
		pr_debug_once("Failed to parse clock data, rc=%d\n", rc);
		goto error_ctrl_put;
	}
...
static int dsi_display_res_deinit(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug_once("clocks deinit failed, rc=%d\n", rc);
					  ...
					  u32 bit_clk_rate)
{
	...
	if (bit_clk_rate == 0) {
		pr_debug_once("Invalid bit clock rate\n");
		return -EINVAL;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (num_of_lanes == 0) {
			pr_debug_once("Invalid lane count\n");
			rc = -EINVAL;
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("Failed to update link frequencies\n");
			goto error;
		}
					...
					struct link_clk_freq *bkp_freq)
{
	...
	if (display->panel->host_config.phy_type == DSI_PHY_TYPE_CPHY) {
		...
		rc = dsi_clk_update_parent(
					...
					&display->clock_info.mux_clks);
		if (rc) {
			pr_debug_once("failed update mux parent to shadow\n");
			goto exit;
		}
	} else {
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("wait4dynamic refresh failed for dsi:%d\n", i);
			goto recover_pix_clk;
		} else {
	...
	if (display->panel->host_config.phy_type == DSI_PHY_TYPE_CPHY) {
		rc = dsi_clk_update_parent(&display->clock_info.cphy_clks,
				&display->clock_info.mux_clks);
		if (rc)
			pr_debug_once("could not switch back to src clks %d\n", rc);
					  ...
					  struct dsi_display_mode *mode)
{
	...
	if (rc) {
		pr_debug_once("failed set link frequencies %d\n", rc);
		goto exit;
	}
		...
		int clk_rate)
{
	...
	if (clk_rate <= 0) {
		pr_debug_once("%s: bitrate should be greater than 0\n", __func__);
		return -EINVAL;
	}
	...
	if (!rc) {
		...
		pr_debug_once("%s: Failed to prepare to configure '%d'. rc = %d\n",
				   ...
				   struct dsi_display_mode *dsi_mode)
{
	...
	if (!dfps_caps.dfps_support && !dyn_clk_caps->maintain_const_fps) {
		pr_debug_once("dfps or constant fps not supported\n");
		return -ENOTSUPP;
	}

	if (dfps_caps.type == DSI_DFPS_IMMEDIATE_CLK) {
		pr_debug_once("dfps clock method not supported\n");
		return -ENOTSUPP;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to dfps update host_%d, rc=%d\n",
				display->name, i, rc);
		goto error;
	}
		...
		u32 *b_fp_out)
{
	...
	if (!a_total || !new_fps) {
		pr_debug_once("Invalid pixel total or new fps in mode request\n");
		return -EINVAL;
	}
	...
	if (b_fp_new < 0) {
		pr_debug_once("Invalid new_hfp calcluated%d\n", b_fp_new);
		return -EINVAL;
	}
				...
				u32 curr_refresh_rate)
{
	...
	if (!dfps_caps.dfps_support) {
		pr_debug_once("dfps not supported by panel\n");
		return -EINVAL;
	}
	...
	if (!curr_refresh_rate) {
		if (!dsi_display_is_seamless_dfps_possible(display,
			...
			pr_debug_once("seamless dynamic fps not supported for mode\n");
			return -EINVAL;
		}
		if (display->panel->cur_mode) {
			...
			pr_debug_once("cur_mode is not initialized\n");
			return -EINVAL;
		}
	}
	...
	default:
		pr_debug_once("Unsupported DFPS mode %d\n", dfps_caps.type);
		rc = -ENOTSUPP;
	}
				    ...
				    u32 flags)
{
	...
	if (!priv_info) {
		pr_debug_once("[%s] failed to get private info of the display mode",
			display->name);
		return -EINVAL;
	}
	...
	rc = dsi_panel_get_host_cfg_for_mode(display->panel,
					     ...
					     &display->config);
	if (rc) {
		pr_debug_once("[%s] failed to get host config for mode, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	if (mode->dsi_mode_flags &
		...
		if (rc) {
			pr_debug_once("[%s]DSI dfps update failed, rc=%d\n",
					display->name, rc);
			goto error;
		}
		...
		display_for_each_ctrl(i, display) {
			...
			if (rc) {
				pr_debug_once("failed to update ctrl config\n");
				goto error;
			}
		}
		...
		if (priv_info->phy_timing_len) {
			display_for_each_ctrl(i, display) {
				...
				if (rc)
					pr_debug_once("Fail to add timing params\n");
			}
		}
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DYN_CLK) {
		if (display->panel->panel_mode == DSI_OP_VIDEO_MODE) {
			...
			if (rc)
				pr_debug_once("dynamic clk change failed %d\n", rc);
			/*
			...
			rc = dsi_display_dynamic_clk_configure_cmd(display,
					clk_rate);
			if (rc) {
				pr_debug_once("Failed to configure dynamic clk\n");
				return rc;
			}
		}
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to update ctrl config, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
		...
		display_for_each_ctrl(i, display) {
			ctrl = &display->ctrl[i];
			 rc = dsi_phy_set_timing_params(ctrl->phy,
				...
				priv_info->phy_timing_len);
			if (rc)
				pr_debug_once("failed to add DSI PHY timing params");
		}
	}
error:
	return rc;
...
static int _dsi_display_dev_init(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to parse dt, rc=%d\n", display->name, rc);
		goto error;
	}

	rc = dsi_display_res_init(display);
	if (rc) {
		pr_debug_once("[%s] failed to initialize resources, rc=%d\n",
		       display->name, rc);
		goto error;
	}
error:
...
static int _dsi_display_dev_deinit(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to deinitialize resource, rc=%d\n",
		       display->name, rc);
...
int dsi_display_cont_splash_config(void *dsi_display)
{
	...
	if (!display) {
		pr_debug_once("invalid input display param\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("failed to vote gdsc for continuous splash, rc=%d\n",
							rc);
		mutex_unlock(&display->display_lock);
		return -EINVAL;
	}
	...
	if (!display->is_cont_splash_enabled) {
		pr_debug_once("Continuous splash is not enabled\n");
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_ON);
	if (rc) {
		pr_debug_once("[%s] failed to enable DSI link clocks, rc=%d\n",
		       display->name, rc);
	...
	if (rc) {
		pr_debug_once("[%s] failed to enable vregs, rc=%d\n",
...
int dsi_display_splash_res_cleanup(struct  dsi_display *display)
{
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_ALL_CLKS, DSI_CLK_OFF);
	if (rc)
		pr_debug_once("[%s] failed to disable DSI link clocks, rc=%d\n",
		       display->name, rc);

	rc = sde_power_resource_enable(display->phandle,
			display->cont_splash_client, false);
	if (rc)
		pr_debug_once("failed to remove vote on gdsc for continuous splash, rc=%d\n",
				rc);
	...
	enum dsi_clk_type clk_type, enum dsi_clk_state clk_state)
{
	...
	if (rc)
		pr_debug_once("%s: failed set clk state, rc = %d\n", __func__, rc);
	mutex_unlock(&dsi_display_clk_mutex);
...
static int dsi_display_force_update_dsi_clk(struct dsi_display *display)
{
	...
	if (!rc) {
		...
		pr_debug_once("Failed to configure dsi bit clock '%d'. rc = %d\n",
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rc) {
		pr_debug_once("%s: kstrtoint failed. rc=%d\n", __func__, rc);
		return rc;
	}

	if (display->panel->panel_mode != DSI_OP_CMD_MODE) {
		pr_debug_once("only supported for command mode\n");
		return -ENOTSUPP;
	}
...
static int dsi_display_validate_split_link(struct dsi_display *display)
{
	...
	if (display->panel->panel_mode == DSI_OP_CMD_MODE) {
		pr_debug_once("[%s] split link is not supported in command mode\n",
			display->name);
		rc = -ENOTSUPP;
		goto error;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (!ctrl->ctrl->split_link_supported) {
			pr_debug_once("[%s] split link is not supported by hw\n",
				display->name);
			rc = -ENOTSUPP;
			goto error;
		}
...
static int dsi_display_sysfs_init(struct dsi_display *display)
{
	...
	if (rc)
		pr_debug_once("failed to create display device attributes");
		...
		void *data)
{
	...
	if (!dev || !pdev || !master) {
		pr_debug_once("invalid param(s), dev %pK, pdev %pK, master %pK\n",
				dev, pdev, master);
		return -EINVAL;
	}
	...
	if (!drm || !display) {
		pr_debug_once("invalid param(s), drm %pK, display %pK\n",
				drm, display);
		return -EINVAL;
	}
	...
	for (i = 0; i < display->panel->host_config.ext_bridge_num; i++) {
		...
		if (!display->ext_bridge[j].node_of) {
			pr_debug_once("invalid ext bridge node\n");
			return -EINVAL;
		}
	...
	if (rc) {
		pr_debug_once("[%s] split link validation failed, rc=%d\n",
						 display->name, rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("[%s] debugfs init failed, rc=%d\n", display->name, rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("[%s] sysfs init failed, rc=%d\n", display->name, rc);
		goto error;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to initialize ctrl[%d], rc=%d\n",
			       display->name, i, rc);
		...
		if (rc) {
			pr_debug_once("[%s] Failed to initialize phy[%d], rc=%d\n",
				display->name, i, rc);
		...
		if (IS_ERR_OR_NULL(info.c_clks[i].dsi_core_client)) {
			pr_debug_once("[%s] client creation failed for ctrl[%d]",
					dsi_client_name, i);
			goto error_ctrl_deinit;
		}
	}
	...
	if (IS_ERR_OR_NULL(display->clk_mngr)) {
		...
		pr_debug_once("dsi clock registration failed, rc = %d\n", rc);
		goto error_ctrl_deinit;
	}
	...
	if (IS_ERR_OR_NULL(handle)) {
		rc = PTR_ERR(handle);
		pr_debug_once("failed to register %s client, rc = %d\n",
		       client1, rc);
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to register ctrl clk_cb[%d], rc=%d\n",
			       display->name, i, rc);
			goto error_ctrl_deinit;
		}
		...
		if (rc) {
			pr_debug_once("[%s] failed to register phy clk_cb[%d], rc=%d\n",
			       display->name, i, rc);
			goto error_ctrl_deinit;
		}
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to initialize mipi host, rc=%d\n",
		       display->name, rc);
		goto error_ctrl_deinit;
	}
	...
	if (rc) {
		if (rc != -EPROBE_DEFER)
			pr_debug_once("[%s] failed to initialize panel driver, rc=%d\n",
			       display->name, rc);
		goto error_host_deinit;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to set phy clk freq, rc=%d\n",
					display->name, rc);
			goto error;
		}
	}
		...
		struct device *master, void *data)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to deinit panel driver, rc=%d\n",
		       display->name, rc);

	rc = dsi_display_mipi_host_deinit(display);
	if (rc)
		pr_debug_once("[%s] failed to deinit mipi hosts, rc=%d\n",
		       display->name,
		       rc);
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc)
			pr_debug_once("[%s] failed to deinit phy%d driver, rc=%d\n",
			       display->name, i, rc);

		rc = dsi_ctrl_drv_deinit(display_ctrl->ctrl);
		if (rc)
			pr_debug_once("[%s] failed to deinit ctrl%d driver, rc=%d\n",
			       display->name, i, rc);
	}
...
static int dsi_display_init(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("device init failed, rc=%d\n", rc);
		goto end;
	}

	rc = component_add(&pdev->dev, &dsi_display_comp_ops);
	if (rc)
		pr_debug_once("component add failed, rc=%d\n", rc);
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (!name) {
			pr_debug_once("display name not defined\n");
			continue;
		}
		...
		struct drm_encoder *enc)
{
	...
	if (!priv) {
		pr_debug_once("Private data is not present\n");
		rc = -EINVAL;
		goto error;
	}

	if (display->bridge) {
		pr_debug_once("display is already initialize\n");
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(bridge)) {
		rc = PTR_ERR(bridge);
		pr_debug_once("[%s] brige init failed, %d\n", display->name, rc);
		goto error;
	}
			   ...
			   struct mipi_dsi_device *dsi)
{
	...
	if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO) {
		...
		pr_debug_once("command mode not supported by ext bridge\n");
		return -ENOTSUPP;
	}
		...
		struct drm_encoder *encoder, struct drm_connector *connector)
{
	...
	for (i = 0; i < display->panel->host_config.ext_bridge_num; i++) {
		...
		if (IS_ERR_OR_NULL(ext_bridge)) {
			...
			pr_debug_once("failed to find ext bridge\n");
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("[%s] ext brige attach failed, %d\n",
				display->name, rc);
			goto error;
		}
		...
		struct msm_display_info *info, void *disp)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to get panel phy props, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	case DSI_OP_CMD_MODE:
		...
		break;
	default:
		pr_debug_once("unknwown dsi panel mode %d\n",
				display->panel->panel_mode);
		break;
	}
...
{
	if (!display || !display->panel) {
		pr_debug_once("invalid display:%d panel:%d\n", display != NULL,
					   ...
					   int start, int end, u32 *mode_idx)
{
	...
	for (i = 1; i < dyn_clk_caps->bit_clk_list_len; i++) {
		...
		for (j = start; j < end; j++) {
			...
			if (!src || !dst) {
				pr_debug_once("invalid mode index\n");
				return;
			}
			  ...
			  struct dsi_display_mode **out_modes)
{
	...
	*out_modes = NULL;
	...
	if (rc) {
		pr_debug_once("[%s] failed to get dfps caps from panel\n",
				display->name);
		goto error;
	}
	...
	for (mode_idx = 0; mode_idx < timing_mode_count; mode_idx++) {
		...
		rc = dsi_panel_get_mode(display->panel, mode_idx,
						...
						topology_override);
		if (rc) {
			pr_debug_once("[%s] failed to get mode idx %d from panel\n",
		...
		for (i = 0; i < num_dfps_rates; i++) {
			...
			if (!sub_mode) {
				pr_debug_once("invalid mode data\n");
				rc = -EFAULT;
				goto error;
			}
	...
	int h_active, int v_active)
{
	...
	if (!refresh_rate) {
		mutex_unlock(&display->display_lock);
		pr_debug_once("Null Refresh Rate\n");
		return -EINVAL;
	}
		...
		struct dsi_display_mode **out_mode)
{
	...
	*out_mode = NULL;
	...
	if (!*out_mode) {
		pr_debug_once("[%s] failed to find mode for v_active %u h_active %u fps %u pclk %u\n",
		...
		if (cur_mode->pixel_clk_khz != adj_mode->pixel_clk_khz) {
			if (dyn_clk_caps->dyn_clk_support) {
				...
				if ((adj_mode->dsi_mode_flags &
					DSI_MODE_FLAG_VRR) &&
					(!dyn_clk_caps->maintain_const_fps)) {
					pr_debug_once("dfps and dyn clk concurrent\n");
					rc = -ENOTSUPP;
					goto error;
				}
			      ...
			      u32 flags)
{
	...
	if (rc) {
		pr_debug_once("[%s] panel mode validation failed, rc=%d\n",
		       display->name, rc);
		goto error;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] ctrl mode validation failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("[%s] phy mode validation failed, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	}
	...
	if ((flags & DSI_VALIDATE_FLAG_ALLOW_ADJUST) &&
		...
		if (rc) {
			pr_debug_once("[%s] seamless not possible rc=%d\n",
				display->name, rc);
			goto error;
		}
	}
			 ...
			 u32 flags)
{
	...
	if (rc) {
		pr_debug_once("[%s] mode cannot be set\n", display->name);
		goto error;
	}

	rc = dsi_display_set_mode_sub(display, &adj_mode, flags);
	if (rc) {
		pr_debug_once("[%s] failed to set mode\n", display->name);
		goto error;
	}

	pr_debug_once("mdp_transfer_time_us=%d us\n",
			adj_mode.priv_info->mdp_transfer_time_us);
	pr_debug_once("hactive= %d,vactive= %d,fps=%d\n",
...
int dsi_display_set_tpg_state(struct dsi_display *display, bool enable)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to set tpg state for host_%d\n",
			       display->name, i);
			goto error;
		}
	}
...
static int dsi_display_pre_switch(struct dsi_display *display)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to update DSI controller, rc=%d\n",
			   display->name, rc);
	...
	if (rc) {
		pr_debug_once("[%s] failed to set DSI link clock source, rc=%d\n",
			display->name, rc);
		goto error_ctrl_deinit;
	}
...
static void dsi_display_register_error_handler(struct dsi_display *display)
{
	...
	if (!display->err_workq) {
		pr_debug_once("failed to create dsi workq!\n");
		return;
	}
...
int dsi_display_prepare(struct dsi_display *display)
{
	...
	if (!display->panel->cur_mode) {
		pr_debug_once("no valid mode set for the display");
		return -EINVAL;
	}
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		if (display->is_cont_splash_enabled) {
			pr_debug_once("DMS is not supposed to be set on first frame\n");
			rc = -EINVAL;
			goto error;
		}
		...
		if (rc)
			pr_debug_once("[%s] panel pre-prepare-res-switch failed, rc=%d\n",
					display->name, rc);
		goto error;
	}
	...
	if (!(mode->dsi_mode_flags & DSI_MODE_FLAG_POMS) &&
		...
		if (rc) {
			pr_debug_once("[%s] panel pre-prepare failed, rc=%d\n",
					display->name, rc);
			goto error;
		}
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to setup DSI controller, rc=%d\n",
		       display->name, rc);
		goto error_phy_disable;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to enable DSI host, rc=%d\n",
		       display->name, rc);
		goto error_ctrl_deinit;
	}
	...
	if (!display->is_cont_splash_enabled) {
		...
		if (rc) {
			pr_debug_once("[%s] failed soft reset, rc=%d\n",
					display->name, rc);
		...
		if (!(mode->dsi_mode_flags & DSI_MODE_FLAG_POMS)) {
			...
			if (rc) {
				pr_debug_once("[%s] panel prepare failed, rc=%d\n",
						display->name, rc);
				goto error_ctrl_link_off;
			}
		}
		...
		struct dsi_rect *out_roi)
{
	...
	if (req_rois->num_rects > roi_caps->num_roi) {
		pr_debug_once("request for %d rois greater than max %d\n",
				req_rois->num_rects,
				roi_caps->num_roi);
		rc = -EINVAL;
		goto exit;
...
static int dsi_display_qsync(struct dsi_display *display, bool enable)
{
	...
	if (!display->panel->qsync_min_fps) {
		pr_debug_once("%s:ERROR: qsync set, but no fps\n", __func__);
		return 0;
	}
	...
	display_for_each_ctrl(i, display) {
		...
		if (enable) {
			...
			if (rc) {
				pr_debug_once("fail qsync ON cmds rc:%d\n", rc);
				goto exit;
			}
		} else {
			/* send the commands to enable qsync */
			...
			if (rc) {
				pr_debug_once("fail qsync OFF cmds rc:%d\n", rc);
				goto exit;
			}
		}
		...
		struct msm_roi_list *rois)
{
	...
	display_for_each_ctrl(i, display) {
		...
		if (rc) {
			pr_debug_once("dsi_display_calc_ctrl_roi failed rc %d\n", rc);
			return rc;
		}

		rc = dsi_ctrl_set_roi(ctrl->ctrl, &ctrl_roi, &changed);
		if (rc) {
			pr_debug_once("dsi_ctrl_set_roi failed rc %d\n", rc);
			return rc;
		}
		...
		if (rc) {
			pr_debug_once("dsi_panel_set_roi failed rc %d\n", rc);
			return rc;
		}
		...
		rc = dsi_ctrl_setup(ctrl->ctrl);
		if (rc) {
			pr_debug_once("dsi_ctrl_setup failed rc %d\n", rc);
			return rc;
		}
	}
...
int dsi_display_config_ctrl_for_cont_splash(struct dsi_display *display)
{
	...
	if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
		...
		if (rc) {
			pr_debug_once("[%s]failed to enable DSI video engine, rc=%d\n",
			       display->name, rc);
			goto error_out;
		}
		...
		rc = dsi_display_cmd_engine_enable(display);
		if (rc) {
			pr_debug_once("[%s]failed to enable DSI cmd engine, rc=%d\n",
			       display->name, rc);
			goto error_out;
		}
	} else {
		pr_debug_once("[%s] Invalid configuration\n", display->name);
		rc = -EINVAL;
	}
		...
		struct msm_display_conn_params *params)
{
	...
	if (params->qsync_update) {
		...
		if (rc)
			pr_debug_once("%s failed to send qsync commands\n",
				__func__);
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (display->is_cont_splash_enabled) {
		...
		if (rc) {
			pr_debug_once("Continuous splash res cleanup failed, rc=%d\n",
				rc);
			return -EINVAL;
		}
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to switch DSI panel mode, rc=%d\n",
				   display->name, rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("[%s] failed to enable DSI panel, rc=%d\n",
			       display->name, rc);
			goto error;
		}
	...
	if (mode->priv_info->dsc_enabled) {
		...
		if (rc) {
			pr_debug_once("[%s] panel pps cmd update failed, rc=%d\n",
				display->name, rc);
			goto error;
		}
	}
...
int dsi_display_post_enable(struct dsi_display *display)
{
	...
	if (display->panel->cur_mode->dsi_mode_flags & DSI_MODE_FLAG_POMS) {
		...
		if (rc)
			pr_debug_once("[%s] panel post-enable failed, rc=%d\n",
			       display->name, rc);
	}
...
int dsi_display_pre_disable(struct dsi_display *display)
{
	...
	if (display->poms_pending) {
		...
		if (rc)
			pr_debug_once("[%s] panel pre-disable failed, rc=%d\n",
			       display->name, rc);
	}
...
int dsi_display_disable(struct dsi_display *display)
{
	...
	rc = dsi_display_wake_up(display);
	if (rc)
		pr_debug_once("[%s] display wake up failed, rc=%d\n",
		       display->name, rc);

	if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
		rc = dsi_display_vid_engine_disable(display);
		if (rc)
			pr_debug_once("[%s]failed to disable DSI vid engine, rc=%d\n",
			       display->name, rc);
	} else if (display->config.panel_mode == DSI_OP_CMD_MODE) {
	...
	if (!display->poms_pending) {
		...
		if (rc)
			pr_debug_once("[%s] failed to disable DSI panel, rc=%d\n",
			       display->name, rc);
	}
...
int dsi_display_update_pps(char *pps_cmd, void *disp)
{
	...
	if (pps_cmd == NULL || disp == NULL) {
		pr_debug_once("Invalid parameter\n");
		return -EINVAL;
	}
...
int dsi_display_unprepare(struct dsi_display *display)
{
	...
	if (!display->poms_pending) {
		...
		if (rc)
			pr_debug_once("[%s] panel unprepare failed, rc=%d\n",
			       display->name, rc);
	}
	...
	{	if (!display->poms_pending) {
			...
			if (rc)
				pr_debug_once("[%s] panel post-unprepare failed, rc=%d\n",
					display->name, rc);
		}
	}
	rc = dsi_display_ctrl_host_disable(display);
	if (rc)
		pr_debug_once("[%s] failed to disable DSI host, rc=%d\n",
		       display->name, rc);

	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_LINK_CLK, DSI_CLK_OFF);
	if (rc)
		pr_debug_once("[%s] failed to disable Link clocks, rc=%d\n",
		       display->name, rc);

	rc = dsi_display_ctrl_deinit(display);
	if (rc)
		pr_debug_once("[%s] failed to deinit controller, rc=%d\n",
		       display->name, rc);
	...
	rc = dsi_display_clk_ctrl(display->dsi_clk_handle,
			DSI_CORE_CLK, DSI_CLK_OFF);
	if (rc)
		pr_debug_once("[%s] failed to disable DSI clocks, rc=%d\n",
		       display->name, rc);
