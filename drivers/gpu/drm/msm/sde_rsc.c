	...
	enum sde_rsc_client_type client_type, u32 vsync_source)
{
	...
	if (!client_name) {
		pr_debug_once("client name is null- not supported\n");
		return ERR_PTR(-EINVAL);
	} else if (rsc_index >= MAX_RSC_COUNT) {
		pr_debug_once("invalid rsc index\n");
		return ERR_PTR(-EINVAL);
	} else if (!rsc_prv_list[rsc_index]) {
		pr_debug_once("rsc not probed yet or not available\n");
		return NULL;
	}
...
void sde_rsc_client_destroy(struct sde_rsc_client *client)
{
	...
	if (state != SDE_RSC_IDLE_STATE) {
		...
		if (wait_vblank_crtc_id != SDE_RSC_INVALID_CRTC_ID) {
			pr_debug_once("unexpected sleep required on crtc %d at rsc client destroy\n",
					wait_vblank_crtc_id);
		...
		void (*cb_func)(uint32_t event_type, void *usr), void *usr)
{
	...
	if (rsc_index >= MAX_RSC_COUNT) {
		pr_debug_once("invalid rsc index:%d\n", rsc_index);
		return ERR_PTR(-EINVAL);
	} else if (!rsc_prv_list[rsc_index]) {
		pr_debug_once("rsc idx:%d not probed yet or not available\n",
								rsc_index);
		return ERR_PTR(-EINVAL);
	} else if (!cb_func || !event_type) {
		pr_debug_once("no event or cb func\n");
		return ERR_PTR(-EINVAL);
	}
	...
	struct sde_power_client *pclient, bool enable)
{
	...
	if (!phandle || !pclient) {
		pr_debug_once("invalid input argument\n");
		return -EINVAL;
	}
	...
	if (enable) {
		rc = msm_dss_enable_vreg(mp->vreg_config, mp->num_vreg,
				enable);
		if (rc) {
			pr_debug_once("failed to enable vregs rc=%d\n", rc);
			goto end;
		}

		rc = msm_dss_enable_clk(mp->clk_config, mp->num_clk, enable);
		if (rc) {
			pr_debug_once("clock enable failed rc:%d\n", rc);
		...
		rc = msm_dss_enable_vreg(mp->vreg_config, mp->num_vreg,
				enable);
		if (rc) {
			pr_debug_once("failed to disable vregs rc=%d\n", rc);
			goto end;
		}
	}
	...
	struct sde_rsc_cmd_config *cmd_config, enum sde_rsc_state state)
{
	...
	if (total < 0) {
		pr_debug_once("invalid total time period time:%llu jiter_time:%llu blanking time:%llu\n",
	...
	int *wait_vblank_crtc_id)
{
	...
	if (!rsc->primary_client) {
		pr_debug_once("primary client not available for cmd state switch\n");
		rc = -EINVAL;
		goto end;
	} else if (caller_client != rsc->primary_client) {
		pr_debug_once("primary client state:%d not cmd state request\n",
...
vsync_wait:
	...
	if (!rc && (rsc->current_state == SDE_RSC_VID_STATE ||
		...
		if (!wait_vblank_crtc_id) {
			pr_debug_once("invalid crtc id wait pointer, client %d\n",
					caller_client->id);
		...
		int *wait_vblank_crtc_id)
{
	...
	if (!rc && rsc->primary_client &&
		...
		if (rsc->hw_ops.hw_vsync)
			rsc->hw_ops.hw_vsync(rsc, VSYNC_ENABLE, NULL, 0, 0);
		if (!wait_vblank_crtc_id) {
			pr_debug_once("invalid crtc id wait pointer provided\n");
			msleep(PRIMARY_VBLANK_WORST_CASE_MS);
		} else {
			*wait_vblank_crtc_id = rsc->primary_client->crtc_id;
		...
		if (!rc) {
			pr_debug_once("Timeout waiting for vsync\n");
			rc = -ETIMEDOUT;
	...
	int *wait_vblank_crtc_id)
{
	...
	if (!rsc->primary_client) {
		pr_debug_once("primary client not available for vid state switch\n");
		rc = -EINVAL;
		goto end;
	} else if (caller_client != rsc->primary_client) {
		pr_debug_once("primary client state:%d not vid state request\n",
			rsc->primary_client->current_state);
		rc = -EINVAL;
		goto end;
	}
		...
		struct sde_rsc_client *caller_client)
{
	...
	if (!caller_client) {
		pr_debug_once("invalid client for rsc state update\n");
		return -EINVAL;
...
static int sde_rsc_hw_init(struct sde_rsc_priv *rsc)
{
	...
	if (ret) {
		pr_debug_once("sde rsc: fs on failed ret:%d\n", ret);
		goto sde_rsc_fail;
	}
	...
	if (ret) {
		pr_debug_once("failed to enable sde rsc power resources\n");
		goto sde_rsc_fail;
	}
	...
	int *wait_vblank_crtc_id)
{
	...
	if (rsc->master_drm == NULL) {
		pr_debug_once("invalid master component binding\n");
		rc = -EINVAL;
		goto end;
	...
	default:
		pr_debug_once("invalid state handling %d\n", state);
		break;
	}
		...
		u32 bus_id, u64 ab_vote, u64 ib_vote)
{
	...
	if (caller_client && caller_client->rsc_index >= MAX_RSC_COUNT) {
		pr_debug_once("invalid rsc client or client index\n");
		return -EINVAL;
	}
	...
	bool delta_vote)
{
	...
	if (delta_vote) {
		if (rsc->hw_ops.tcs_wait) {
			...
			if (rc) {
				pr_debug_once("tcs is still busy; can't send command\n");
...
static int _sde_debugfs_status_show(struct seq_file *s, void *data)
{
	...
	if (rsc->hw_ops.debug_show) {
		...
		if (ret)
			pr_debug_once("sde rsc: hw debug failed ret:%d\n", ret);
	}
			...
			const char __user *p, size_t count, loff_t *ppos)
{
	...
	if (rc) {
		pr_debug_once("mode_state: int conversion failed rc:%d\n", rc);
		goto end;
	}
	...
	if (mode_state != ALL_MODES_DISABLED &&
		...
		pr_debug_once("invalid mode:%d combination\n", mode_state);
		goto end;
	}
			...
			const char __user *p, size_t count, loff_t *ppos)
{
	...
	if (rc) {
		pr_debug_once("vsync_state: int conversion failed rc:%d\n", rc);
		goto end;
	}
		...
		void *data)
{
	...
	if (!drm || !rsc) {
		pr_debug_once("invalid param(s), drm %pK, rsc %pK\n",
				drm, rsc);
		return -EINVAL;
	}
		...
		struct device *master, void *data)
{
	...
	rsc = platform_get_drvdata(pdev);
	if (!rsc) {
		pr_debug_once("invalid display rsc\n");
		return;
	}
...
static int sde_rsc_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		pr_debug_once("sde rsc:power resource init failed ret:%d\n", ret);
		goto sde_rsc_fail;
	}
	...
	if (IS_ERR_OR_NULL(rsc->pclient)) {
		...
		rsc->pclient = NULL;
		pr_debug_once("sde rsc:power client create failed ret:%d\n", ret);
		goto sde_rsc_fail;
	}
	...
	if (IS_ERR_OR_NULL(rsc->disp_rsc)) {
		...
		rsc->disp_rsc = NULL;
		pr_debug_once("sde rsc:get display rsc failed ret:%d\n", ret);
		goto sde_rsc_fail;
	}

	ret = msm_dss_ioremap_byname(pdev, &rsc->wrapper_io, "wrapper");
	if (ret) {
		pr_debug_once("sde rsc: wrapper io data mapping failed ret=%d\n", ret);
		goto sde_rsc_fail;
	}

	ret = msm_dss_ioremap_byname(pdev, &rsc->drv_io, "drv");
	if (ret) {
		pr_debug_once("sde rsc: drv io data mapping failed ret:%d\n", ret);
		goto sde_rsc_fail;
	}
	...
	if (IS_ERR_OR_NULL(rsc->fs)) {
		rsc->fs = NULL;
		pr_debug_once("unable to get regulator\n");
		goto sde_rsc_fail;
	}
	...
	if (ret) {
		pr_debug_once("sde rsc: hw register failed ret:%d\n", ret);
		goto sde_rsc_fail;
	}
