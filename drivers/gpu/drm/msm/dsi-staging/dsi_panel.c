...
static int dsi_panel_vreg_get(struct dsi_panel *panel)
{
	...
	for (i = 0; i < panel->power_info.count; i++) {
		...
		rc = PTR_RET(vreg);
		if (rc) {
			pr_debug_once("failed to get %s regulator\n",
			       panel->power_info.vregs[i].vreg_name);
			goto error_put;
		}
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->reset_gpio)) {
		...
		if (rc) {
			pr_debug_once("request for reset_gpio failed, rc=%d\n", rc);
			goto error;
		}
	}
	...
	if (gpio_is_valid(r_config->disp_en_gpio)) {
		...
		if (rc) {
			pr_debug_once("request for disp_en_gpio failed, rc=%d\n", rc);
	...
	if (gpio_is_valid(panel->bl_config.en_gpio)) {
		...
		if (rc) {
			pr_debug_once("request for bklt_en_gpio failed, rc=%d\n", rc);
	...
	if (gpio_is_valid(r_config->lcd_mode_sel_gpio)) {
		...
		if (rc) {
			pr_debug_once("request for mode_gpio failed, rc=%d\n", rc);
	...
	if (gpio_is_valid(panel->hbm_config.hbm_en_gpio)) {
		...
		if (rc) {
			pr_debug_once("request hbm gpio failed, rc=%d\n", rc);
...
int dsi_panel_trigger_esd_attack(struct dsi_panel *panel)
{
	...
	if (!panel) {
		pr_debug_once("Invalid panel param\n");
		return -EINVAL;
	}

	r_config = &panel->reset_config;
	if (!r_config) {
		pr_debug_once("Invalid panel reset configuration\n");
		return -EINVAL;
	}
	...
	pr_debug_once("failed to pull down gpio\n");
	return -EINVAL;
}
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->reset_config.disp_en_gpio)) {
		...
		if (rc) {
			pr_debug_once("unable to set dir for disp gpio rc=%d\n", rc);
			goto exit;
		}
	}
	...
	if (r_config->count) {
		...
		if (rc) {
			pr_debug_once("unable to set dir for rst gpio rc=%d\n", rc);
			goto exit;
		}
	}
	...
	if (gpio_is_valid(panel->bl_config.en_gpio)) {
		...
		if (rc)
			pr_debug_once("unable to set dir for bklt gpio rc=%d\n", rc);
	}
	...
	if (gpio_is_valid(panel->reset_config.lcd_mode_sel_gpio)) {
		...
		if (rc)
			pr_debug_once("unable to set dir for mode gpio rc=%d\n", rc);
	}
exit:
	return rc;
}
...
static int dsi_panel_set_pinctrl_state(struct dsi_panel *panel, bool enable)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to set pin state, rc=%d\n", panel->name,
		       rc);
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to set pinctrl, rc=%d\n", panel->name, rc);
		goto error_disable_vregs;
	}

	rc = dsi_panel_reset(panel);
	if (rc) {
		pr_debug_once("[%s] failed to reset panel, rc=%d\n", panel->name, rc);
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed set pinctrl state, rc=%d\n", panel->name,
		       rc);
	}
	if(panel->tp_state_check && panel->lcd_not_sleep){
		pr_info("(%s)+lcd not sleep \n", panel->name);
				...
				enum dsi_cmd_set_type type)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (len < 0) {
			rc = len;
			pr_debug_once("failed to set cmds(%d), rc=%d\n", type, rc);
			goto error;
		}
...
static int dsi_panel_pinctrl_init(struct dsi_panel *panel)
{
	...
	if (IS_ERR_OR_NULL(panel->pinctrl.pinctrl)) {
		...
		pr_debug_once("failed to get pinctrl, rc=%d\n", rc);
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(panel->pinctrl.active)) {
		...
		pr_debug_once("failed to get pinctrl active state, rc=%d\n", rc);
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(panel->pinctrl.suspend)) {
		...
		pr_debug_once("failed to get pinctrl suspend state, rc=%d\n", rc);
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(panel->pinctrl.tp_rst_output_high)) {
		pr_debug_once("failed to get pinctrl tp_rst_output_high state, rc=%d\n", rc);
	}
	...
	u32 bl_lvl)
{
	...
	if (rc < 0)
		pr_debug_once("failed to update dcs backlight:%d\n", bl_lvl);
	...
	u32 bl_lvl)
{
	...
	bl = &panel->bl_config;
	if (!bl->pwm_bl) {
		pr_debug_once("pwm device not found\n");
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to change pwm config, rc=%i\n",
		       panel->name, rc);
		goto error;
	}
	...
	if (!bl->pwm_enabled) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to enable pwm, rc=%i\n",
			       panel->name, rc);
			goto error;
		}
...
static  bool dsi_panel_param_is_hbm_on(struct dsi_panel *panel)
{
	...
	if (!panel_param) {
		pr_debug_once("%s: invalid panel_param.\n", __func__);
		return false;
	}
			...
			if (!bl_lvl) {
				if (gpio_is_valid(panel->hbm_config.hbm_en_gpio)) {
					if (gpio_direction_output(panel->hbm_config.hbm_en_gpio, 0))
						pr_debug_once("[%s] failed to set hbm_en_gpio to low\n",panel->name);
				}
				return false;
			} else
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_PWM:
		...
		break;
	default:
		pr_debug_once("Backlight type(%d) not supported\n", bl->type);
		rc = -ENOTSUPP;
	}
...
static int dsi_panel_pwm_register(struct dsi_panel *panel)
{
	...
	if (IS_ERR_OR_NULL(bl->pwm_bl)) {
		...
		pr_debug_once("[%s] failed to request pwm, rc=%d\n", panel->name,
			rc);
		return rc;
	}
                                ...
                                struct msm_param_info *param_info)
{
	...
	if (panel_param->value == param_info->value)
	{
		...
		if (!param_map_state->cmds || !param_map_state->cmds->cmds) {
			pr_debug_once("Invalid cmds or cmds->cmds\n");
			rc = -EINVAL;
			goto end;
		}
		...
		if (len < 0) {
			rc = len;
			pr_debug_once("%s:failed to send param cmd, rc=%d\n",
					__func__, rc);
			goto end;
		}
                        ...
                        struct msm_param_info *param_info)
{
	...
	if (!gpio_is_valid(panel->hbm_config.hbm_en_gpio)) {
		pr_debug_once("%s: invalid hbm_en_gpio.\n", __func__);
		return -EINVAL;
	}
	...
	switch (param_info->value) {
		...
		case HBM_ON_STATE:
			...
			if (rc < 0)
				pr_debug_once("%s: failed to send param cmds. ret=%d\n", __func__, rc);
			break;
		case HBM_OFF_STATE:
			rc = gpio_direction_output(panel->hbm_config.hbm_en_gpio, 0);
			...
			if (rc < 0) {
				pr_debug_once("%s: failed to set backlight. ret=%d\n", __func__, rc);
				...
				struct msm_param_info *param_info)
{
	...
	if (!panel || !param_info) {
                pr_debug_once("%s: invalid params\n", __func__);
                return -EINVAL;
        }
	...
	switch (param_info->param_idx) {
		...
		case PARAM_ACL_ID :
			...
			break;
		default:
			pr_debug_once("%s: Invalid set_param type=%d\n",
				  ...
				  struct dsi_parser_utils *utils)
{
	...
	rc = utils->read_u32(utils->data,
				...
				&mode->refresh_rate);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-panel-framerate, rc=%d\n",
		       rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-panel-width",
				  &mode->h_active);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-panel-width, rc=%d\n", rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data,
				"qcom,mdss-dsi-h-front-porch",
				  &mode->h_front_porch);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-h-front-porch, rc=%d\n",
		       rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data,
				"qcom,mdss-dsi-h-back-porch",
				  &mode->h_back_porch);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-h-back-porch, rc=%d\n",
		       rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data,
				"qcom,mdss-dsi-h-pulse-width",
				  &mode->h_sync_width);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-h-pulse-width, rc=%d\n",
		       rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-h-sync-skew",
				  &mode->h_skew);
	if (rc)
		pr_debug_once("qcom,mdss-dsi-h-sync-skew is not defined, rc=%d\n", rc);
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-panel-height",
				  &mode->v_active);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-panel-height, rc=%d\n",
		       rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-v-back-porch",
				  &mode->v_back_porch);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-v-back-porch, rc=%d\n",
		       rc);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-v-pulse-width",
				  &mode->v_sync_width);
	if (rc) {
		pr_debug_once("failed to read qcom,mdss-dsi-v-pulse-width, rc=%d\n",
		       rc);
		goto error;
	}
					...
					const char *name)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to read qcom,mdss-dsi-bpp, rc=%d\n",
		       name, rc);
		return rc;
	}
				       ...
				       const char *name)
{
	...
	if (host->data_lanes == 0) {
		pr_debug_once("[%s] No data lanes are enabled, rc=%d\n", name, rc);
		rc = -EINVAL;
	}
				      ...
				      const char *name)
{
	...
	if (swap_mode) {
		if (!strcmp(swap_mode, "rgb_swap_rgb")) {
			...
			pr_debug_once("[%s] Unrecognized color order-%s\n",
			       name, swap_mode);
			rc = -EINVAL;
		}
	} else {
				    ...
				    const char *name)
{
	...
	if (trig) {
		if (!strcmp(trig, "none")) {
			...
			pr_debug_once("[%s] Unrecognized mdp trigger type (%s)\n",
			       name, trig);
			rc = -EINVAL;
		}
...
static int dsi_panel_parse_host_config(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_parse_pixel_format(&panel->host_config, utils,
					  panel->name);
	if (rc) {
		pr_debug_once("[%s] failed to get pixel format, rc=%d\n",
		panel->name, rc);
		goto error;
	}
	...
	rc = dsi_panel_parse_lane_states(&panel->host_config, utils,
					 panel->name);
	if (rc) {
		pr_debug_once("[%s] failed to parse lane states, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
	...
	rc = dsi_panel_parse_color_swap(&panel->host_config, utils,
					panel->name);
	if (rc) {
		pr_debug_once("[%s] failed to parse color swap config, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
	...
	rc = dsi_panel_parse_triggers(&panel->host_config, utils,
				      panel->name);
	if (rc) {
		pr_debug_once("[%s] failed to parse triggers, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
	...
	rc = dsi_panel_parse_misc_host_config(&panel->host_config, utils,
					      panel->name);
	if (rc) {
		pr_debug_once("[%s] failed to parse misc host config, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
	...
	dsi_panel_parse_ext_bridge_config(&panel->host_config, utils,
					      panel->name);
	if (rc) {
		pr_debug_once("[%s] failed to parse ext bridge config, rc=%d\n",
		       panel->name, rc);
	}
				     ...
				     struct device_node *of_node)
{
	...
	rc = of_property_read_u32(of_node,
				  ...
				  &val);
	if (rc)
		pr_debug_once("[%s] qsync min fps not defined rc:%d\n",
			panel->name, rc);
...
static int dsi_panel_parse_dyn_clk_caps(struct dsi_panel *panel)
{
	...
	if (dyn_clk_caps->bit_clk_list_len < 1) {
		pr_debug_once("[%s] failed to get supported bit clk list\n", name);
		return -EINVAL;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to parse supported bit clk list\n", name);
		return -EINVAL;
	}
...
static int dsi_panel_parse_dfps_caps(struct dsi_panel *panel)
{
	...
	if (!type) {
		pr_debug_once("[%s] dfps type not defined\n", name);
		rc = -EINVAL;
		goto error;
		...
		pr_debug_once("[%s] dfps type is not recognized\n", name);
		rc = -EINVAL;
		goto error;
	}
	...
	if (dfps_caps->dfps_list_len < 1) {
		pr_debug_once("[%s] dfps refresh list not present\n", name);
		rc = -EINVAL;
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("[%s] dfps refresh rate list parse failed\n", name);
		rc = -EINVAL;
		goto error;
	}
					     ...
					     const char *name)
{
	...
	if (rc) {
		...
		pr_debug_once("[%s] Unrecognized value for mdss-dsi-h-sync-pulse\n",
		       name);
		rc = -EINVAL;
		goto error;
	}
	...
	if (!traffic_mode) {
		...
		pr_debug_once("[%s] Unrecognized traffic mode-%s\n", name,
		       traffic_mode);
		rc = -EINVAL;
		goto error;
	}
					   ...
					   const char *name)
{
	...
	if (rc) {
		...
		pr_debug_once("[%s] Unrecognized value for mdss-dsi-te-dcs-command\n",
		       name);
		rc = -EINVAL;
		goto error;
	}
...
static int dsi_panel_parse_panel_mode(struct dsi_panel *panel)
{
	...
	if (!mode) {
		...
		pr_debug_once("[%s] Unrecognized panel type-%s\n", panel->name, mode);
		rc = -EINVAL;
		goto error;
	}
	...
	if (panel_mode == DSI_OP_VIDEO_MODE || panel_mode_switch_enabled) {
		rc = dsi_panel_parse_video_host_config(&panel->video_config,
						       ...
						       panel->name);
		if (rc) {
			pr_debug_once("[%s] Failed to parse video host cfg, rc=%d\n",
			       panel->name, rc);
			goto error;
		}
	}
	...
	if (panel_mode == DSI_OP_CMD_MODE || panel_mode_switch_enabled) {
		rc = dsi_panel_parse_cmd_host_config(&panel->cmd_config,
						     ...
						     panel->name);
		if (rc) {
			pr_debug_once("[%s] Failed to parse cmd host config, rc=%d\n",
			       panel->name, rc);
			goto error;
		}
	}
...
static int dsi_panel_parse_phy_props(struct dsi_panel *panel)
{
	...
	if (!str) {
		...
		pr_debug_once("[%s] Unrecognized panel rotation-%s\n", name, str);
		rc = -EINVAL;
		goto error;
	}
error:
					...
					struct dsi_parser_utils *utils)
{
	...
	if (rc) {
		pr_debug_once("commands failed, rc=%d\n", rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("failed to allocate cmd packets, rc=%d\n", rc);
		goto error;
	}
	...
	rc = dsi_panel_create_cmd_packets(data, length, packet_count,
					  cmd->cmds);
	if (rc) {
		pr_debug_once("failed to create cmd packets, rc=%d\n", rc);
		goto error_free_mem;
	}
	...
	if (!state || !strcmp(state, "dsi_lp_mode")) {
		...
		pr_debug_once("[%s] command state unrecognized-%s\n",
		...
		struct dsi_parser_utils *utils)
{
	...
	if (!priv_info) {
		pr_debug_once("invalid mode priv info\n");
		return -EINVAL;
	}
	...
	for (i = DSI_CMD_SET_PRE_ON; i < DSI_CMD_SET_MAX; i++) {
		...
		if (i == DSI_CMD_SET_PPS) {
			...
			if (rc)
				pr_debug_once("failed to allocate cmd set %d, rc = %d\n",
					i, rc);
...
static int dsi_panel_parse_reset_sequence(struct dsi_panel *panel)
{
	...
	if (!arr) {
		pr_debug_once("[%s] dsi-reset-sequence not found\n", panel->name);
		rc = -EINVAL;
		goto error;
	}
	if (length & 0x1) {
		pr_debug_once("[%s] syntax error for dsi-reset-sequence\n",
		       panel->name);
		rc = -EINVAL;
		goto error;
	}

	pr_debug_once("RESET SEQ LENGTH = %d\n", length);
	...
	rc = utils->read_u32_array(utils->data, "qcom,mdss-dsi-reset-sequence",
					arr_32, length);
	if (rc) {
		pr_debug_once("[%s] cannot read dso-reset-seqience\n", panel->name);
...
static int dsi_panel_parse_power_cfg(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to parse vregs\n", panel->name);
		goto error;
	}
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.reset_gpio) &&
		...
		pr_debug_once("[%s] failed get reset gpio, rc=%d\n", panel->name, rc);
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to parse reset sequence, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
...
static int dsi_panel_parse_bl_pwm_config(struct dsi_panel *panel)
{
	...
	rc = utils->read_u32(utils->data, "qcom,bl-pmic-pwm-period-usecs",
				  &val);
	if (rc) {
		pr_debug_once("bl-pmic-pwm-period-usecs is not defined, rc=%d\n", rc);
		goto error;
	}
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	if (panel->bl_config.type == DSI_BACKLIGHT_PWM) {
		...
		if (rc) {
			pr_debug_once("[%s] failed to parse pwm config, rc=%d\n",
			       panel->name, rc);
			goto error;
		}
	}
...
void dsi_dsc_pclk_param_calc(struct msm_display_dsc_info *dsc, int intf_width)
{
	...
	if (!dsc || !dsc->slice_width || !dsc->slice_per_pkt ||
		...
		pr_debug_once("invalid input, intf_width=%d slice_width=%d\n",
				...
				struct dsi_parser_utils *utils)
{
	...
	if (rc) {
		...
		if (priv_info->dsc.version != 0x11) {
			pr_debug_once("%s: DSC version:%d not supported\n", __func__,
	...
	if (rc) {
		...
		if (priv_info->dsc.scr_rev > 0x1) {
			pr_debug_once("%s: DSC scr version:%d not supported\n",
	...
	if (rc) {
		pr_debug_once("failed to parse qcom,mdss-dsc-slice-height\n");
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("failed to parse qcom,mdss-dsc-slice-width\n");
		goto error;
	}
	...
	if (intf_width % priv_info->dsc.slice_width) {
		pr_debug_once("invalid slice width for the intf width:%d slice width:%d\n",
	...
	if (rc) {
		pr_debug_once("failed to parse qcom,mdss-dsc-slice-per-pkt\n");
		goto error;
	} else if (!data || (data > 2)) {
		pr_debug_once("invalid dsc slice-per-pkt:%d\n", data);
		goto error;
	}
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsc-bit-per-component",
		&data);
	if (rc) {
		pr_debug_once("failed to parse qcom,mdss-dsc-bit-per-component\n");
		goto error;
	}
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsc-bit-per-pixel",
			&data);
	if (rc) {
		pr_debug_once("failed to parse qcom,mdss-dsc-bit-per-pixel\n");
		goto error;
	}
...
static int dsi_panel_parse_hdr_config(struct dsi_panel *panel)
{
	...
	if (hdr_prop->hdr_enabled) {
		...
		if (rc) {
			pr_debug_once("%s:%d, Unable to read color primaries,rc:%u",
		...
		if (rc) {
			pr_debug_once("%s:%d, Unable to read hdr brightness, rc:%u",
				__func__, __LINE__, rc);
			hdr_prop->hdr_enabled = false;
			return rc;
		}
		...
		int topology_override)
{
	...
	if (len <= 0 || len % TOPOLOGY_SET_LEN ||
		...
		pr_debug_once("invalid topology list for the panel, rc = %d\n", rc);
		return rc;
	}
	...
	if (rc) {
		pr_debug_once("unable to read the display topologies, rc = %d\n", rc);
		goto read_fail;
	}
	...
	if (rc) {
		pr_debug_once("no default topology selected, rc = %d\n", rc);
		goto parse_fail;
	}

	if (top_sel >= top_count) {
		rc = -EINVAL;
		pr_debug_once("default topology is specified is not valid, rc = %d\n",
			rc);
		goto parse_fail;
	}
					 ...
					 struct msm_roi_alignment *align)
{
	...
	len /= sizeof(u32);
	if (!data) {
		pr_debug_once("panel roi alignment not found\n");
		rc = -EINVAL;
	} else if (len != 6) {
		pr_debug_once("incorrect roi alignment len %d\n", len);
		rc = -EINVAL;
	} else {
		rc = utils->read_u32_array(utils->data,
				...
				struct dsi_parser_utils *utils)
{
	...
	if (!mode || !mode->priv_info) {
		pr_debug_once("invalid arguments\n");
		return -EINVAL;
	}
...
static int dsi_panel_parse_dms_info(struct dsi_panel *panel)
{
	...
	if (data && !strcmp(data, "dynamic-resolution-switch-immediate")) {
		...
		pr_debug_once("[%s] unsupported dynamic switch mode: %s\n",
							panel->name, data);
		return -EINVAL;
	}
	...
	char *prop_key, u32 **target, u32 cmd_cnt)
{
	...
	tmp /= sizeof(u32);
	if (tmp != cmd_cnt) {
		pr_debug_once("request property(%d) do not match cmd count(%d)\n",
				tmp, cmd_cnt);
		return false;
	}
	...
	*target = kcalloc(tmp, sizeof(u32), GFP_KERNEL);
	if (IS_ERR_OR_NULL(*target)) {
		pr_debug_once("Error allocating memory for property\n");
		return false;
	}

	if (utils->read_u32_array(utils->data, prop_key, *target, tmp)) {
		pr_debug_once("cannot get values from dts\n");
		kfree(*target);
		*target = NULL;
		return false;
	}
...
int dsi_panel_parse_esd_reg_read_configs(struct dsi_panel *panel)
{
	...
	if (!esd_config->status_cmd.count) {
		pr_debug_once("panel status command parsing failed\n");
		rc = -EINVAL;
		goto error;
	}
	...
	if (!dsi_panel_parse_esd_status_len(utils,
		...
		pr_debug_once("Invalid status read length\n");
		rc = -EINVAL;
		goto error1;
	}
	...
	if (!IS_ERR_OR_NULL(data) && tmp != 0 && (tmp % status_len) == 0) {
		...
		pr_debug_once("error parse panel-status-value\n");
		rc = -EINVAL;
		goto error2;
	}
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	if (!rc) {
		if (!strcmp(string, "bta_check")) {
			...
			if (panel->panel_mode == DSI_OP_CMD_MODE) {
				...
				pr_debug_once("TE-ESD not valid for video mode\n");
				rc = -EINVAL;
				goto error;
			}
		} else {
			pr_debug_once("No valid panel-status-check-mode string\n");
			rc = -EINVAL;
			goto error;
		}
	} else {
	...
	if (panel->esd_config.status_mode == ESD_MODE_REG_READ) {
		...
		if (rc) {
			pr_debug_once("failed to parse esd reg read mode params, rc=%d\n",
						rc);
			goto error;
		}
	...
	for (i = 0; i < PARAM_ID_NUM; i++) {
		...
		for (j = 0; j < param->val_max && param->val_max > 0 ; j++) {
			...
			rc = dsi_panel_parse_cmd_sets_sub(param_map->cmds, type,
								utils);
			if (rc) {
				pr_debug_once("%s: panel param cmd %s parsing failed\n", __func__,
						param->param_name);
				break;
			}
		}
					...
					struct device_node *of_node)
{
	...
	if (rc) {
		...
		pr_debug_once("[%s] panel off nosleep cmd state not defined rc:%d, default as %d\n",
...
static int dsi_panel_get_pwr_mode(struct dsi_panel *panel, u8 *val)
{
	...
	if (len < 0) {
		pr_debug_once("failed to transfer pwr_mode cmd\n");
		goto err;
	}
	*val = rx_buf & 0xFF;
				...
				int topology_override)
{
	...
	if (!pname || strlen(pname) == 0) {
		pr_warn("Failed to get qcom,mdss-dsi-panel-supplier\n");
	...
	if (rc) {
		pr_debug_once("failed to parse host configuration, rc=%d\n", rc);
		goto error;
	}

	rc = dsi_panel_parse_panel_mode(panel);
	if (rc) {
		pr_debug_once("failed to parse panel mode configuration, rc=%d\n", rc);
		goto error;
	}

	rc = dsi_panel_parse_dfps_caps(panel);
	if (rc)
		pr_debug_once("failed to parse dfps configuration, rc=%d\n", rc);
	...
	if (rc)
		pr_debug_once("failed to parse dynamic clk config, rc=%d\n", rc);

	rc = dsi_panel_parse_phy_props(panel);
	if (rc) {
		pr_debug_once("failed to parse panel physical dimension, rc=%d\n", rc);
		goto error;
	}

	rc = dsi_panel_parse_gpios(panel);
	if (rc) {
		pr_debug_once("failed to parse panel gpios, rc=%d\n", rc);
		goto error;
	}

	rc = dsi_panel_parse_power_cfg(panel);
	if (rc)
		pr_debug_once("failed to parse power config, rc=%d\n", rc);

	rc = dsi_panel_parse_bl_config(panel);
	if (rc) {
		pr_debug_once("failed to parse backlight config, rc=%d\n", rc);
		if (rc == -EPROBE_DEFER)
			goto error;
	}
	...
	if (rc)
		pr_debug_once("failed to parse misc features, rc=%d\n", rc);

	rc = dsi_panel_parse_hdr_config(panel);
	if (rc)
		pr_debug_once("failed to parse hdr config, rc=%d\n", rc);

	rc = dsi_panel_get_mode_count(panel);
	if (rc) {
		pr_debug_once("failed to get mode count, rc=%d\n", rc);
		goto error;
	}
		       ...
		       struct mipi_dsi_host *host)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to get panel regulators, rc=%d\n",
		       panel->name, rc);
		goto exit;
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to init pinctrl, rc=%d\n", panel->name, rc);
		goto error_vreg_put;
	}

	rc = dsi_panel_gpio_request(panel);
	if (rc) {
		pr_debug_once("[%s] failed to request gpios, rc=%d\n", panel->name,
		       rc);
	...
	if (rc) {
		if (rc != -EPROBE_DEFER)
			pr_debug_once("[%s] failed to register backlight, rc=%d\n",
			       panel->name, rc);
...
int dsi_panel_drv_deinit(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to unregister backlight, rc=%d\n",
		       panel->name, rc);

	rc = dsi_panel_gpio_release(panel);
	if (rc)
		pr_debug_once("[%s] failed to release gpios, rc=%d\n", panel->name,
		       rc);

	rc = dsi_panel_pinctrl_deinit(panel);
	if (rc)
		pr_debug_once("[%s] failed to deinit gpios, rc=%d\n", panel->name,
		       rc);

	rc = dsi_panel_vreg_put(panel);
	if (rc)
		pr_debug_once("[%s] failed to put regs, rc=%d\n", panel->name, rc);
...
int dsi_panel_get_mode_count(struct dsi_panel *panel)
{
	...
	if (!timings_np && !panel->host_config.ext_bridge_num) {
		pr_debug_once("no display timing nodes defined\n");
		rc = -EINVAL;
		goto error;
	}
	...
	if ((!count && !panel->host_config.ext_bridge_num) ||
		count > DSI_MODE_MAX) {
		pr_debug_once("invalid count of timing nodes: %d\n", count);
		rc = -EINVAL;
		goto error;
	}
			...
			int topology_override)
{
	...
	dsi_for_each_child_node(timings_np, child_np) {
		...
		if (rc) {
			pr_debug_once("failed to parse panel timing, rc=%d\n", rc);
			goto parse_fail;
		}

		rc = dsi_panel_parse_dsc_params(mode, utils);
		if (rc) {
			pr_debug_once("failed to parse dsc params, rc=%d\n", rc);
			goto parse_fail;
		}
		...
		rc = dsi_panel_parse_topology(prv_info, utils,
				topology_override);
		if (rc) {
			pr_debug_once("failed to parse panel topology, rc=%d\n", rc);
			goto parse_fail;
		}

		rc = dsi_panel_parse_cmd_sets(prv_info, utils);
		if (rc) {
			pr_debug_once("failed to parse command sets, rc=%d\n", rc);
			goto parse_fail;
		}
		...
		if (rc)
			pr_debug_once("failed to partial update caps, rc=%d\n", rc);
		...
		if (panel->panel_mode_switch_enabled) {
			...
			if (rc) {
				pr_debug_once("PMS: failed to parse panel mode\n");
				rc = 0;
...
int dsi_panel_pre_prepare(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] panel power on failed, rc=%d\n", panel->name, rc);
		goto error;
	}
...
int dsi_panel_update_pps(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to send DSI_CMD_SET_PPS cmds, rc=%d\n",
			panel->name, rc);
	}
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_LP1 cmd, rc=%d\n",
		       panel->name, rc);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_set_lp2(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_LP2 cmd, rc=%d\n",
		       panel->name, rc);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_NOLP cmd, rc=%d\n",
		       panel->name, rc);
exit:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_set_tearing(struct dsi_panel *panel, bool enable)
{
	...
	len =  ops->transfer(panel->host, &cmd.msg);
	if (len < 0) {
		pr_debug_once("failed to transfer 0x%x cmd\n", payload);
		goto err;
	}
err:
	return rc;
...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to send DSI_CMD_SET_PRE_ON cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
		...
		int ctrl_idx)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_QSYNC_ON cmds rc=%d\n",
		       panel->name, rc);
		...
		int ctrl_idx)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_QSYNC_OFF cmds rc=%d\n",
		       panel->name, rc);
		...
		struct dsi_rect *roi)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to prepare DSI_CMD_SET_ROI cmds, rc=%d\n",
				panel->name, rc);
		return rc;
	}
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_ROI cmds, rc=%d\n",
				panel->name, rc);
...
int dsi_panel_pre_mode_switch_to_video(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_CMD_TO_VID_SWITCH cmds, rc=%d\n",
			panel->name, rc);
	mutex_unlock(&panel->panel_lock);
...
int dsi_panel_pre_mode_switch_to_cmd(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_VID_TO_CMD_SWITCH cmds, rc=%d\n",
			panel->name, rc);
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_mode_switch_to_cmd(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_POST_VID_TO_CMD_SWITCH cmds, rc=%d\n",
			panel->name, rc);
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_mode_switch_to_vid(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_POST_CMD_TO_VID_SWITCH cmds, rc=%d\n",
			panel->name, rc);
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
int dsi_panel_get_elvss_data(struct dsi_panel *panel)
{
	...
	if (rc < 0)
		pr_debug_once("failed to get elvss data\n");
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_TIMING_SWITCH cmds, rc=%d\n",
		       panel->name, rc);
...
int dsi_panel_post_switch(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug_once("[%s] failed to send DSI_CMD_SET_POST_TIMING_SWITCH cmds, rc=%d\n",
		       panel->name, rc);
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to send DSI_CMD_SET_ON cmds, rc=%d\n",
		       panel->name, rc);
		goto err;
	}
	...
	if (!panel->no_panel_on_read_support) {
		...
		if (rc) {
			pr_debug_once("[%s] Failed to read pwr_mode, rc = %d",
							panel->name, rc);
			goto err;
		}

		if (pwr_mode != panel->disp_on_chk_val) {
			pr_debug_once("%s: Read Pwr_mode=0%x is not matched with expected value =0x%x\n",
				__func__, pwr_mode, panel->disp_on_chk_val);
			if (panel_recovery_retry++ > 5) {
				pr_debug_once("%s: panel recovery failed for all retries",
						__func__);
...
int dsi_panel_post_enable(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to send DSI_CMD_SET_POST_ON cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
error:
...
int dsi_panel_pre_disable(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->hbm_config.hbm_en_gpio)) {
		rc = gpio_direction_output(panel->hbm_config.hbm_en_gpio, 0);
		if (rc) {
			pr_debug_once("[%s] failed to set hbm_en_gpio to low, rc=%d\n",
				panel->name, rc);
			goto error;
		}
	}
	...
	if (rc) {
		pr_debug_once("[%s] failed to send DSI_CMD_SET_PRE_OFF cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
...
int dsi_panel_unprepare(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] failed to send DSI_CMD_SET_POST_OFF cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
...
int dsi_panel_post_unprepare(struct dsi_panel *panel)
{
	...
	if (rc) {
		pr_debug_once("[%s] panel power_Off failed, rc=%d\n",
		       panel->name, rc);
		goto error;
	}
