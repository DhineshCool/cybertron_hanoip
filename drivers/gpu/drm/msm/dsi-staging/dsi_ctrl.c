				     ...
				     loff_t *ppos)
{
	...
	if (rc) {
		pr_debug_once("failed to enable DSI core clocks\n");
		kfree(buf);
		return rc;
	}
	...
	rc = dsi_ctrl->clk_cb.dsi_clk_cb(dsi_ctrl->clk_cb.priv, clk_info);
	if (rc) {
		pr_debug_once("failed to disable DSI core clocks\n");
		kfree(buf);
		return rc;
	}
				 ...
				 struct dentry *parent)
{
	...
	if (IS_ERR_OR_NULL(dir)) {
		rc = PTR_ERR(dir);
		pr_debug_once("[DSI_%d] debugfs create dir failed, rc=%d\n",
	...
	if (IS_ERR_OR_NULL(state_file)) {
		...
		pr_debug_once("[DSI_%d] state file failed, rc=%d\n",
		       dsi_ctrl->cell_index, rc);
	...
	if (IS_ERR_OR_NULL(reg_dump)) {
		rc = PTR_ERR(reg_dump);
		pr_debug_once("[DSI_%d] reg dump file failed, rc=%d\n",
		       dsi_ctrl->cell_index, rc);
		goto error_remove_dir;
	}
				...
				u32 op_state)
{
	...
	case DSI_CTRL_OP_CMD_ENGINE:
		if (state->cmd_engine_state == op_state) {
			pr_debug_once("[%d] No change in state, cmd_state=%d\n",
			       dsi_ctrl->cell_index, op_state);
			rc = -EINVAL;
		} else if ((state->power_state != DSI_CTRL_POWER_VREG_ON) ||
			   (state->controller_state != DSI_CTRL_ENGINE_ON)) {
			pr_debug_once("[%d]State error: op=%d: %d, %d\n",
			       dsi_ctrl->cell_index,
			       op,
			       state->power_state,
	...
	case DSI_CTRL_OP_HOST_ENGINE:
		if (state->controller_state == op_state) {
			pr_debug_once("[%d] No change in state, ctrl_state=%d\n",
			       dsi_ctrl->cell_index, op_state);
			rc = -EINVAL;
		} else if (state->power_state != DSI_CTRL_POWER_VREG_ON) {
			pr_debug_once("[%d]State error (link is off): op=%d:, %d\n",
			       dsi_ctrl->cell_index,
			       op_state,
			       state->power_state);
			rc = -EINVAL;
			...
			pr_debug_once("[%d]State error (eng on): op=%d: %d, %d\n",
				  dsi_ctrl->cell_index,
				  op_state,
	...
	case DSI_CTRL_OP_CMD_TX:
		if ((state->power_state != DSI_CTRL_POWER_VREG_ON) ||
			...
			pr_debug_once("[%d]State error: op=%d: %d, %d, %d\n",
			       dsi_ctrl->cell_index,
			       op,
			       state->power_state,
	...
	case DSI_CTRL_OP_HOST_INIT:
		if (state->host_initialized == op_state) {
			pr_debug_once("[%d] No change in state, host_init=%d\n",
			       dsi_ctrl->cell_index, op_state);
			rc = -EINVAL;
		} else if (state->power_state != DSI_CTRL_POWER_VREG_ON) {
			pr_debug_once("[%d]State error: op=%d: %d\n",
	...
	case DSI_CTRL_OP_TPG:
		if (state->tpg_enabled == op_state) {
			pr_debug_once("[%d] No change in state, tpg_enabled=%d\n",
			       dsi_ctrl->cell_index, op_state);
			rc = -EINVAL;
		} else if ((state->power_state != DSI_CTRL_POWER_VREG_ON) ||
			   (state->controller_state != DSI_CTRL_ENGINE_ON)) {
	...
	case DSI_CTRL_OP_ASYNC_TIMING:
		if (state->vid_engine_state != op_state) {
			pr_debug_once("[%d] Unexpected engine state vid_state=%d\n",
			       dsi_ctrl->cell_index, op_state);
			rc = -EINVAL;
		}
		break;
...
bool dsi_ctrl_validate_host_state(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (!state) {
		pr_debug_once("Invalid host state for DSI controller\n");
		return -EINVAL;
	}
				...
				struct dsi_ctrl *ctrl)
{
	...
	case DSI_CTRL_VERSION_2_0:
		...
		if (IS_ERR(ptr)) {
			pr_debug_once("mmss_misc base address not found for [%s]\n",
					ctrl->name);
			rc = PTR_ERR(ptr);
			return rc;
		}
	...
	case DSI_CTRL_VERSION_2_4:
		...
		if (IS_ERR(ptr)) {
			pr_debug_once("disp_cc base address not found for [%s]\n",
					ctrl->name);
			rc = PTR_ERR(ptr);
			return rc;
		}
				...
				struct dsi_ctrl *ctrl)
{
	...
	if (IS_ERR(hs_link->byte_clk)) {
		...
		pr_debug_once("failed to get byte_clk, rc=%d\n", rc);
	...
	if (IS_ERR(hs_link->pixel_clk)) {
		...
		pr_debug_once("failed to get pixel_clk, rc=%d\n", rc);
	...
	if (IS_ERR(lp_link->esc_clk)) {
		...
		pr_debug_once("failed to get esc_clk, rc=%d\n", rc);
	...
	if (IS_ERR(rcg->byte_clk)) {
		...
		pr_debug_once("failed to get byte_clk_rcg, rc=%d\n", rc);
		rcg->byte_clk = NULL;
		goto fail;
	}
	...
	if (IS_ERR(rcg->pixel_clk)) {
		...
		pr_debug_once("failed to get pixel_clk_rcg, rc=%d\n", rc);
		rcg->pixel_clk = NULL;
		goto fail;
	}
...
static int dsi_ctrl_supplies_deinit(struct dsi_ctrl *ctrl)
{
	...
	for (i = 0; i < regs->count; i++) {
		if (!regs->vregs[i].vreg)
			pr_debug_once("vreg is NULL, should not reach here\n");
		else
				  ...
				  struct dsi_ctrl *ctrl)
{
	...
	if (rc) {
		pr_debug_once("failed to get host power supplies, rc = %d\n", rc);
		goto error_digital;
	}
					...
					struct dsi_ctrl *ctrl)
{
	...
	if (!bus->bus_handle) {
		rc = -EINVAL;
		pr_debug_once("failed to register axi bus client\n");
	}
					...
					struct dsi_host_config *config)
{
	...
	if (config->panel_mode >= DSI_OP_MODE_MAX) {
		pr_debug_once("Invalid dsi operation mode (%d)\n", config->panel_mode);
		rc = -EINVAL;
		goto err;
	}

	if ((host_cfg->data_lanes & (DSI_CLOCK_LANE - 1)) == 0) {
		pr_debug_once("No data lanes are enabled\n");
		rc = -EINVAL;
		goto err;
	}
err:
...
static int dsi_ctrl_enable_supplies(struct dsi_ctrl *dsi_ctrl, bool enable)
{
	...
	if (enable) {
		if (!dsi_ctrl->current_state.host_initialized) {
			...
			if (rc) {
				pr_debug_once("failed to enable host power regs\n");
				goto error;
			}
		}
		...
		rc = dsi_pwr_enable_regulator(&dsi_ctrl->pwr_info.digital,
					      true);
		if (rc) {
			pr_debug_once("failed to enable gdsc, rc=%d\n", rc);
		...
		rc = dsi_pwr_enable_regulator(&dsi_ctrl->pwr_info.digital,
					      false);
		if (rc) {
			pr_debug_once("failed to disable gdsc, rc=%d\n", rc);
			goto error;
		}
		...
		if (!dsi_ctrl->current_state.host_initialized) {
			...
			if (rc) {
				pr_debug_once("failed to disable host power regs\n");
				goto error;
			}
		}
	}
		...
		u32 *flags)
{
	...
	if (*flags & DSI_CTRL_CMD_FIFO_STORE) {
		...
		if ((cmd_len + 4) > DSI_CTRL_MAX_CMD_FIFO_STORE_SIZE) {
			pr_debug_once("Cannot transfer Cmd in FIFO config\n");
			return -ENOTSUPP;
		}
		if (!dsi_ctrl->hw.ops.kickoff_fifo_command) {
			pr_debug_once("Cannot transfer command,ops not defined\n");
			return -ENOTSUPP;
		}
	}
	...
	if (*flags & DSI_CTRL_CMD_NON_EMBEDDED_MODE) {
		if (*flags & DSI_CTRL_CMD_BROADCAST) {
			pr_debug_once("Non embedded not supported with broadcast\n");
			return -ENOTSUPP;
		}
		...
		if ((cmd_len + 4) > SZ_4K) {
			pr_debug_once("Cannot transfer,size is greater than 4096\n");
			return -ENOTSUPP;
		}
	}
			  ...
			  u32 flags)
{
	...
	if (rc) {
		pr_debug_once(" Cmd tx validation failed, cannot transfer cmd\n");
		rc = -ENOTSUPP;
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("Failed to create message packet, rc=%d\n", rc);
		goto error;
	}
	...
	rc = dsi_ctrl_copy_and_pad_cmd(dsi_ctrl,
			...
			&length);
	if (rc) {
		pr_debug_once("[%s] failed to copy message, rc=%d\n",
				dsi_ctrl->name, rc);
		goto error;
	}
	...
	if (!(flags & DSI_CTRL_CMD_DEFER_TRIGGER)) {
		...
		if (ret == 0) {
			...
			if (status & mask) {
				...
				dsi_ctrl_disable_status_interrupt(dsi_ctrl,
						DSI_SINT_CMD_MODE_DMA_DONE);
				pr_debug_once("[DSI_%d]Command transfer failed\n",
						dsi_ctrl->cell_index);
			}
		}
				   ...
				   u32 size)
{
	...
	if (rc)
		pr_debug_once("failed to send max return size packet, rc=%d\n", rc);
			  ...
			  u32 flags)
{
	...
	while (!read_done) {
		...
		if (rc) {
			pr_debug_once("Failed to set max return packet size, rc=%d\n",
			       rc);
			goto error;
		}
		...
		rc = dsi_message_tx(dsi_ctrl, msg, flags);
		if (rc) {
			pr_debug_once("Message transmission failed, rc=%d\n", rc);
			goto error;
		}
		/*
	...
	case MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT:
		pr_debug_once("Rx ACK_ERROR 0x%x\n", cmd);
		rc = 0;
		break;
...
static int dsi_enable_ulps(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (rc) {
		pr_debug_once("lanes not entering idle, skip ULPS\n");
		return rc;
	}
	...
	if ((lanes & ulps_lanes) != lanes) {
		pr_debug_once("Failed to enter ULPS, request=0x%x, actual=0x%x\n",
		       lanes, ulps_lanes);
		rc = -EIO;
	}
...
static int dsi_disable_ulps(struct dsi_ctrl *dsi_ctrl)
{
	...
	if ((lanes & ulps_lanes) != lanes)
		pr_debug_once("Mismatch between lanes in ULPS\n");
	...
	ulps_lanes = dsi_ctrl->hw.ops.ulps_ops.get_lanes_in_ulps(&dsi_ctrl->hw);
	if (ulps_lanes & lanes) {
		pr_debug_once("Lanes (0x%x) stuck in ULPS\n", ulps_lanes);
		rc = -EIO;
	}
...
static int dsi_ctrl_buffer_deinit(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (dsi_ctrl->tx_cmd_buf) {
		...
		if (!aspace) {
			pr_debug_once("failed to get address space\n");
			return -ENOMEM;
		}
...
int dsi_ctrl_buffer_init(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (IS_ERR(dsi_ctrl->tx_cmd_buf)) {
		...
		pr_debug_once("failed to allocate gem, rc=%d\n", rc);
		dsi_ctrl->tx_cmd_buf = NULL;
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("failed to get iova, rc=%d\n", rc);
		(void)dsi_ctrl_buffer_deinit(dsi_ctrl);
		goto error;
	}

	if (iova & 0x07) {
		pr_debug_once("Tx command buffer is not 8 byte aligned\n");
		rc = -ENOTSUPP;
		(void)dsi_ctrl_buffer_deinit(dsi_ctrl);
		goto error;
	}
				  ...
				  struct device_node *of_node)
{
	...
	if (!dsi_ctrl || !of_node) {
		pr_debug_once("invalid dsi_ctrl:%d or of_node:%d\n",
...
static int dsi_ctrl_dev_probe(struct platform_device *pdev)
{
	...
	if (rc) {
		pr_debug_once("ctrl:%d dts parse failed, rc = %d\n",
						dsi_ctrl->cell_index, rc);
		goto fail;
	}
	...
	if (rc) {
		pr_debug_once("Failed to parse register information, rc = %d\n", rc);
		goto fail;
	}

	rc = dsi_ctrl_clocks_init(pdev, dsi_ctrl);
	if (rc) {
		pr_debug_once("Failed to parse clock information, rc = %d\n", rc);
		goto fail;
	}

	rc = dsi_ctrl_supplies_init(pdev, dsi_ctrl);
	if (rc) {
		pr_debug_once("Failed to parse voltage supplies, rc = %d\n", rc);
		goto fail_clks;
	}
	...
	if (rc) {
		pr_debug_once("Catalog does not support version (%d)\n",
		       dsi_ctrl->version);
		goto fail_supplies;
	}
...
static int dsi_ctrl_dev_remove(struct platform_device *pdev)
{
	...
	if (rc)
		pr_debug_once("failed to deinitialize axi bus client, rc = %d\n", rc);

	rc = dsi_ctrl_supplies_deinit(dsi_ctrl);
	if (rc)
		pr_debug_once("failed to deinitialize voltage supplies, rc=%d\n", rc);

	rc = dsi_ctrl_clocks_deinit(dsi_ctrl);
	if (rc)
		pr_debug_once("failed to deinitialize clocks, rc=%d\n", rc);
...
struct dsi_ctrl *dsi_ctrl_get(struct device_node *of_node)
{
	...
	if (!ctrl) {
		pr_debug_once("Device with of node not found\n");
...
void dsi_ctrl_put(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (dsi_ctrl->refcount == 0)
		pr_debug_once("Unbalanced %s call\n", __func__);
	else
		dsi_ctrl->refcount--;
...
int dsi_ctrl_drv_init(struct dsi_ctrl *dsi_ctrl, struct dentry *parent)
{
	...
	if (rc) {
		pr_debug_once("Failed to initialize driver state, rc=%d\n", rc);
		goto error;
	}

	rc = dsi_ctrl_debugfs_init(dsi_ctrl, parent);
	if (rc) {
		pr_debug_once("[DSI_%d] failed to init debug fs, rc=%d\n",
		       dsi_ctrl->cell_index, rc);
		goto error;
	}
...
int dsi_ctrl_drv_deinit(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (rc)
		pr_debug_once("failed to release debugfs root, rc=%d\n", rc);

	rc = dsi_ctrl_buffer_deinit(dsi_ctrl);
	if (rc)
		pr_debug_once("Failed to free cmd buffers, rc=%d\n", rc);
...
int dsi_ctrl_phy_sw_reset(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (rc) {
		pr_debug_once("[DSI_%d] Controller state check failed, rc=%d\n",
		       dsi_ctrl->cell_index, rc);
		goto error;
	}
		...
		bool enable)
{
	...
	if (!dsi_ctrl) {
		pr_debug_once("Invalid dsi_ctrl\n");
		return -EINVAL;
	}
					...
					struct dsi_ctrl *dsi_ctrl)
{
	...
	if (!dsi_ctrl) {
		pr_debug_once("Invalid DSI controller structure\n");
		return false;
	}
				...
				unsigned long int error)
{
	...
	/* DTLN PHY error */
	if (error & 0x3000E00)
		pr_debug_once("dsi PHY contention error: 0x%lx\n", error);
	...
	if (error & 0xE0) {
		...
		pr_debug_once("tx timeout error: 0x%lx\n", error);
	}
	...
	if (error & 0xF0000) {
		...
		if (cb_info.event_cb && !(mask & 0xf0000)) {
			...
			(void)cb_info.event_cb(cb_info.event_usr_ptr,
						...
						dsi_ctrl->cell_index,
						0, 0, 0, 0);
			pr_debug_once("dsi FIFO OVERFLOW error: 0x%lx\n", error);
		}
	}
	...
	if (error & 0xF00000) {
		...
		pr_debug_once("dsi FIFO UNDERFLOW error: 0x%lx\n", error);
	}

	/* DSI PLL UNLOCK error */
	if (error & BIT(8))
		pr_debug_once("dsi PLL unlock error: 0x%lx\n", error);

	/* ACK error */
	if (error & 0xF)
		pr_debug_once("ack error: 0x%lx\n", error);
...
static int _dsi_ctrl_setup_isr(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (irq_num < 0) {
		pr_debug_once("[DSI_%d] Failed to get IRQ number, %d\n",
		...
		if (rc) {
			pr_debug_once("[DSI_%d] Failed to request IRQ, %d\n",
					dsi_ctrl->cell_index, rc);
		} else {
...
int dsi_ctrl_host_timing_update(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (dsi_ctrl->host_config.panel_mode == DSI_OP_CMD_MODE) {
		...
		pr_debug_once("invalid panel mode for resolution switch\n");
		return -EINVAL;
	}
...
int dsi_ctrl_host_deinit(struct dsi_ctrl *dsi_ctrl)
{
	...
	if (rc) {
		pr_debug_once("[DSI_%d] Controller state check failed, rc=%d\n",
		       dsi_ctrl->cell_index, rc);
		pr_debug_once("driver state check failed, rc=%d\n", rc);
		goto error;
	}
				...
				int flags, void *clk_handle)
{
	...
	if (rc) {
		pr_debug_once("panel validation failed, rc=%d\n", rc);
		goto error;
	}
	...
	if (!(flags & (DSI_MODE_FLAG_SEAMLESS | DSI_MODE_FLAG_VRR |
		...
		if (rc) {
			pr_debug_once("[%s] failed to update link frequencies, rc=%d\n",
			       ctrl->name, rc);
			goto error;
		}
	}
			  ...
			  u32 flags)
{
	...
	if (flags & DSI_CTRL_CMD_READ) {
		...
		if (rc <= 0)
			pr_debug_once("read message failed read length, rc=%d\n", rc);
	} else {
		rc = dsi_message_tx(dsi_ctrl, msg, flags);
		if (rc)
			pr_debug_once("command msg transfer failed, rc = %d\n", rc);
	}
			     ...
			     enum dsi_power_state state)
{
	...
	if (state == DSI_CTRL_POWER_VREG_ON) {
		...
		if (rc) {
			pr_debug_once("[%d]failed to enable voltage supplies, rc=%d\n",
			       dsi_ctrl->cell_index, rc);
			goto error;
		}
		...
		if (rc) {
			pr_debug_once("[%d]failed to disable vreg supplies, rc=%d\n",
			       dsi_ctrl->cell_index, rc);
			goto error;
		}
	}
...
int dsi_ctrl_set_ulps(struct dsi_ctrl *dsi_ctrl, bool enable)
{
	...
	if (rc) {
		pr_debug_once("[DSI_%d] Ulps state change(%d) failed, rc=%d\n",
		...
		bool enable, bool ulps_enabled)
{
	...
	if (rc) {
		pr_debug_once("[DSI_%d] Failed to enable IO clamp\n",
			dsi_ctrl->cell_index);
		goto error;
	}
			      ...
			      struct dsi_clk_link_set *source_clks)
{
	...
	if (rc) {
		pr_debug_once("[DSI_%d]Failed to update link clk parent, rc=%d\n",
		       dsi_ctrl->cell_index, rc);
