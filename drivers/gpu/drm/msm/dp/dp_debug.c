...
bail:
	...
	if (dp_debug_configure_mst_bridge(debug))
		pr_debug_once("failed to config mst bridge\n");
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (kstrtoint(offset_ch, 16, &offset)) {
		pr_debug_once("offset kstrtoint error\n");
		goto bail;
	}
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (!in_list)
		pr_debug_once("invalid connector id %u\n", con_id);
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (!is_link_rate_valid(max_bw_code)) {
		pr_debug_once("Unsupported bw code %d\n", max_bw_code);
		return len;
	}
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (max_pclk > debug->parser->max_pclk_khz)
		pr_debug_once("requested: %d, max_pclk_khz:%d\n", max_pclk,
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (mst_port_cnt > DP_MST_SIM_MAX_PORTS) {
		pr_debug_once("port cnt:%d exceeding max:%d\n", mst_port_cnt,
		...
		char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (!connector) {
		pr_debug_once("connector is NULL\n");
		rc = -EINVAL;
		goto error;
	}
		...
		char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	if (!in_list) {
		pr_debug_once("connector %u not in mst list\n", debug->mst_con_id);
		rc = -EINVAL;
		goto error;
	}
		...
		char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	list_for_each_entry(mst_connector,
		...
		if (!connector) {
			pr_debug_once("connector for id %d is NULL\n",
...
static int dp_debug_init(struct dp_debug *dp_debug)
{
	...
	if (!IS_ENABLED(CONFIG_DEBUG_FS)) {
		pr_debug_once("Not creating debug root dir\n");
		debug->root = NULL;
		return 0;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs create file failed, rc=%d\n",
		       DEBUG_NAME, rc);
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs create edid_modes failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs create edid_modes_mst failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs create mst_con_id failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs create mst_conn_info failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs hpd failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs connected failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs max_bw_code failed, rc=%d\n",
		       DEBUG_NAME, rc);
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs register failed, rc=%d\n",
		       DEBUG_NAME, rc);
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs edid failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs dpcd failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs tpg failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs hdr failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs sim failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs attention failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs dump failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs max_pclk_khz failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs force_encryption failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs hdcp failed, rc=%d\n",
			DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs hdcp_wait_sink_sync failed, rc=%d\n",
		       DEBUG_NAME, rc);
		goto error_remove_dir;
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs dsc_feature failed, rc=%d\n",
		       DEBUG_NAME, rc);
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs fec_feature_enable failed, rc=%d\n",
		       DEBUG_NAME, rc);
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs widebus failed, rc=%d\n",
		       DEBUG_NAME, rc);
	}
	...
	if (IS_ERR_OR_NULL(file)) {
		rc = PTR_ERR(file);
		pr_debug_once("[%s] debugfs max_lclk_khz failed, rc=%d\n",
		       DEBUG_NAME, rc);
	}
