...
static irqreturn_t dp_display_irq(int irq, void *dev_id)
{
	...
	if (dp_display_is_hdcp_enabled(dp) && dp->hdcp.ops->isr) {
		if (dp->hdcp.ops->isr(dp->hdcp.data))
			pr_debug_once("dp_hdcp_isr failed\n");
	}
...
static void dp_display_hdcp_register_streams(struct dp_display_private *dp)
{
	...
	if (dp_display_is_ready(dp) && dp->mst.mst_active && ops &&
		...
		if (index > 0) {
			...
			if (rc)
				pr_debug_once("failed to register streams. rc = %d\n",
					rc);
		}
	}
}
...
static int dp_display_initialize_hdcp(struct dp_display_private *dp)
{
	...
	if (IS_ERR_OR_NULL(fd)) {
		pr_debug_once("Error initializing HDCP 1.x\n");
		rc = -EINVAL;
		goto error;
	}
	...
	if (IS_ERR_OR_NULL(fd)) {
		pr_debug_once("Error initializing HDCP 2.x\n");
		rc = -EINVAL;
		goto error;
	}
		...
		void *data)
{
	...
	if (!drm || !dp) {
		pr_debug_once("invalid param(s), drm %pK, dp %pK\n",
				drm, dp);
		rc = -EINVAL;
		goto end;
	}
...
static void dp_display_send_hpd_event(struct dp_display_private *dp)
{
	...
	if (!connector) {
		pr_debug_once("connector not set\n");
		return;
	}
						...
						bool mst_probe)
{
	...
	if (!dp->mst.mst_active) {
		...
		if (ret < 0) {
			pr_debug_once("sink mst enablement failed\n");
			return;
		}
...
static int dp_display_usbpd_configure_cb(struct device *dev)
{
	...
	if (!dp) {
		pr_debug_once("no driver data found\n");
		rc = -ENODEV;
		goto end;
	}
	...
	if (!dp->dp_display.drm_dev->mode_config.poll_enabled) {
		...
		if (i == poll_timeout) {
			pr_debug_once("driver is not loaded\n");
			rc = -ENODEV;
			goto end;
		}
	}
...
{
	if (!dp->active_stream_cnt) {
		pr_debug_once("invalid active_stream_cnt (%d)\n",
...
static int dp_display_get_usb_extcon(struct dp_display_private *dp)
{
	...
	if (rc)
		pr_debug_once("failed to register for usb event: %d\n", rc);
...
static int dp_init_sub_modules(struct dp_display_private *dp)
{
	...
	if (IS_ERR(dp->parser)) {
		...
		pr_debug_once("failed to initialize parser, rc = %d\n", rc);
		dp->parser = NULL;
		goto error;
	}
	...
	if (rc) {
		pr_debug_once("device tree parsing failed\n");
		goto error_catalog;
	}
	...
	if (IS_ERR(dp->catalog)) {
		...
		pr_debug_once("failed to initialize catalog, rc = %d\n", rc);
		dp->catalog = NULL;
		goto error_catalog;
	}
	...
	if (IS_ERR(dp->power)) {
		rc = PTR_ERR(dp->power);
		pr_debug_once("failed to initialize power, rc = %d\n", rc);
		dp->power = NULL;
		goto error_power;
	}
	...
	if (rc) {
		pr_debug_once("Power client create failed\n");
		goto error_aux;
	}
	...
	if (IS_ERR(dp->aux)) {
		rc = PTR_ERR(dp->aux);
		pr_debug_once("failed to initialize aux, rc = %d\n", rc);
		dp->aux = NULL;
		goto error_aux;
	}
	...
	if (rc) {
		pr_debug_once("DRM DP AUX register failed\n");
		goto error_link;
	}
	...
	if (IS_ERR(dp->link)) {
		rc = PTR_ERR(dp->link);
		pr_debug_once("failed to initialize link, rc = %d\n", rc);
		dp->link = NULL;
		goto error_link;
	}
	...
	if (IS_ERR(dp->panel)) {
		rc = PTR_ERR(dp->panel);
		pr_debug_once("failed to initialize panel, rc = %d\n", rc);
		dp->panel = NULL;
		goto error_panel;
	}
	...
	if (IS_ERR(dp->ctrl)) {
		rc = PTR_ERR(dp->ctrl);
		pr_debug_once("failed to initialize ctrl, rc = %d\n", rc);
		dp->ctrl = NULL;
		goto error_ctrl;
	}
	...
	if (IS_ERR(dp->panel->audio)) {
		...
		pr_debug_once("failed to initialize audio, rc = %d\n", rc);
		dp->panel->audio = NULL;
		goto error_audio;
	}
	...
	if (IS_ERR(dp->hpd)) {
		rc = PTR_ERR(dp->hpd);
		pr_debug_once("failed to initialize hpd, rc = %d\n", rc);
		dp->hpd = NULL;
		goto error_hpd;
	}
	...
	if (IS_ERR(dp->debug)) {
		rc = PTR_ERR(dp->debug);
		pr_debug_once("failed to initialize debug, rc = %d\n", rc);
		dp->debug = NULL;
		goto error_debug;
	}
	...
	if (rc) {
		pr_debug_once("failed register hpd\n");
		goto error_hpd_reg;
	}
		...
		struct dp_display_mode *mode)
{
	...
	if (!dp_panel->connector) {
		pr_debug_once("invalid connector input\n");
		return -EINVAL;
	}
			...
			u32 num_slots, u32 pbn, int vcpi)
{
	...
	if (strm_id >= DP_STREAM_MAX) {
		pr_debug_once("invalid stream id:%d\n", strm_id);
		return -EINVAL;
	}

	if (start_slot + num_slots > max_slots) {
		pr_debug_once("invalid channel info received. start:%d, slots:%d\n",
...
static int dp_display_enable(struct dp_display *dp_display, void *panel)
{
	...
	if (!dp->core_initialized) {
		pr_debug_once("host not initialized\n");
		goto end;
	}
...
static int dp_display_post_enable(struct dp_display *dp_display, void *panel)
{
	...
	if (!dp_display_is_ready(dp) || !dp->core_initialized) {
		pr_debug_once("display not ready\n");
		goto end;
	}
...
static int dp_request_irq(struct dp_display *dp_display)
{
	...
	if (dp->irq < 0) {
		rc = dp->irq;
		pr_debug_once("failed to get irq: %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_debug_once("failed to request IRQ%u: %d\n",
				dp->irq, rc);
		return rc;
	}
	disable_irq(dp->irq);
		...
		void *panel, struct drm_display_mode *mode)
{
	...
	dp_panel = panel;
	if (!dp_panel->connector) {
		pr_debug_once("invalid connector\n");
		goto end;
	}
...
static int dp_display_create_workqueue(struct dp_display_private *dp)
{
	...
	if (IS_ERR_OR_NULL(dp->wq)) {
		pr_debug_once("Error creating wq\n");
		return -EPERM;
	}
...
static int dp_display_usbpd_get(struct dp_display_private *dp)
{
	...
	if (IS_ERR(dp->pd)) {
		...
		pr_debug_once("usbpd phandle failed (%ld)\n", PTR_ERR(dp->pd));
	}
	return rc;
}
...
static int dp_display_init_aux_switch(struct dp_display_private *dp)
{
	...
	if (!dp->pdev->dev.of_node) {
		pr_debug_once("cannot find dev.of_node\n");
		rc = -ENODEV;
		goto end;
	}
	...
	if (rc) {
		pr_debug_once("failed to register notifier (%d)\n", rc);
		goto end;
	}
...
static int dp_display_init_aux_bridge(struct dp_display_private *dp)
{
	...
	if (!dp->aux_bridge) {
		pr_debug_once("failed to find dp aux bridge\n");
		rc = -EPROBE_DEFER;
		goto end;
	}
		...
		struct drm_connector *connector)
{
	...
	if (IS_ERR(dp_panel->audio)) {
		...
		pr_debug_once("[mst] failed to initialize audio, rc = %d\n", rc);
		dp_panel->audio = NULL;
		mutex_unlock(&dp->session_lock);
		return rc;
	}
			...
			struct drm_connector *connector)
{
	...
	if (!sde_conn->drv_panel) {
		pr_debug_once("invalid panel for connector:%d\n", connector->base.id);
		mutex_unlock(&dp->session_lock);
		return -EINVAL;
	}
	...
	switch (dev_id) {
		...
		case 0x64:
			*addr = 0x0e;
			break;
		default:
			pr_debug_once("%s unsupported devid 0x%x\n", __func__, dev_id);
			return -1;
	}
							...
							unsigned char data, unsigned char mask)
{
	...
	if (dp_get_sink_addr(dev_id, &buf[0])) {
		pr_debug_once("%s get devid fail 0x%x\n", dev_id);
		return -1;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!dev || !pdev) {
		pr_debug_once("%s invalid param(s), dev %pK, pdev %pK\n",
				__func__, dev, pdev);
		return 0;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cnt == 3) {
		...
		if (ret)
			pr_debug_once("%s write failed %d\n", __func__, ret);
	} else {
		pr_debug_once("%s data len %d, do not need writing sink\n", __func__, cnt);
	}
...
static int dp_display_probe(struct platform_device *pdev)
{
	...
	if (rc) {
		pr_debug_once("Failed to create workqueue\n");
		goto error;
	}
...
int dp_display_get_displays(void **displays, int count)
{
	...
	if (count != 1) {
		pr_debug_once("invalid number of displays\n");
		return -EINVAL;
	}
...
static int __init dp_display_init(void)
{
	...
	if (ret) {
		pr_debug_once("driver register failed");
		return ret;
	}
