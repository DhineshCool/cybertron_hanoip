		...
		char *base_addr, char *addr, size_t len_bytes, u32 **dump_mem)
{
	...
	if (in_mem) {
		...
		if (dump_mem && *dump_mem) {
			...
			in_mem = 0;
			pr_debug_once("dump_mem: kzalloc fails!\n");
		}
	}
	...
	if (_sde_power_check(sde_dbg_base.dump_mode)) {
		...
		if (rc) {
			pr_debug_once("failed to enable power %d\n", rc);
			return;
		}
	}
	...
	u32 reg_dump_flag, bool dump_secure)
{
	...
	if (!dbg || !(dbg->base || dbg->cb)) {
		pr_debug_once("dbg base is null!\n");
		return;
	}
...
static void _sde_dbg_dump_sde_dbg_bus(struct sde_dbg_sde_debug_bus *bus)
{
	...
	if (!mem_base) {
		pr_debug_once("unable to find mem_base for %s\n", bus->cmn.name);
		return;
	}
	...
	if (in_mem) {
		...
		if (*dump_mem) {
			...
			in_mem = false;
			pr_debug_once("dump_mem: allocation fails\n");
		}
	}
...
void sde_dbg_dump(enum sde_dbg_dump_context dump_mode, const char *name, ...)
{
	...
	while ((blk_name = va_arg(args, char*))) {
		if (i++ >= SDE_EVTLOG_MAX_DATA) {
			pr_debug_once("could not parse all dump arguments\n");
			break;
		}
		...
		if (blk_base) {
			if (index < blk_len) {
				...
				pr_debug_once("insufficient space to to dump %s\n",
						blk_name);
			}
		}
...
void sde_dbg_ctrl(const char *name, ...)
{
	...
	while ((blk_name = va_arg(args, char*))) {
		if (i++ >= SDE_EVTLOG_MAX_DATA) {
			pr_debug_once("could not parse all dbg arguments\n");
			break;
		}
		...
		size_t count, loff_t *ppos)
{
	...
	if (len < 0 || len > count) {
		pr_debug_once("len is more than user buffer size");
		return 0;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if ((count < sizeof(buf)) || copy_to_user(buff, buf, len)) {
		pr_debug_once("error copying the buffer! count:0x%zx\n", count);
		return -EFAULT;
	}
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (!file) {
		pr_debug_once("DbgDbg: %s: error no file --\n", __func__);
		return -EINVAL;
	}
	...
	if (kstrtouint(buf, 0, &dbg_ctrl)) {
		pr_debug_once("%s: error in the number of bytes\n", __func__);
		return -EFAULT;
	}
		...
		void *start, int count, char *buf, int buflen)
{
	...
	if (!start || !buf) {
		pr_debug_once("invalid address for dump\n");
		return len;
	}

	if (buflen < PAGE_SIZE) {
		pr_debug_once("buffer too small for dump\n");
		return len;
	}
		...
		char  *buf, int buflen)
{
	...
	if (!sub_blk || (buflen < PAGE_SIZE)) {
		pr_debug_once("invalid params buflen:%d subblk valid:%d\n",
				buflen, sub_blk != NULL);
		return len;
	}
	...
	if (count < DUMP_CLMN_COUNT) {
		pr_debug_once("invalid count for register dumps :%d\n", count);
		return len;
	}
...
static bool sde_dbg_reg_base_is_valid_range(u32 off, u32 cnt)
{
	...
	pr_debug_once("invalid range requested\n");
	return false;
}
			...
			char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(user_buf, dbg->buf + *ppos, len)) {
		mutex_unlock(&sde_dbg_base.mutex);
		pr_debug_once("failed to copy to user\n");
		return -EFAULT;
	}
...
int sde_dbg_debugfs_register(struct device *dev)
{
	...
	if (!ddev) {
		pr_debug_once("Invalid drm device node\n");
		return -EINVAL;
	}

	priv = ddev->dev_private;
	if (!priv) {
		pr_debug_once("Invalid msm drm private node\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_root)) {
		pr_debug_once("debugfs_root create_dir fail, error %ld\n",
...
void sde_dbg_init_dbg_buses(u32 hwversion)
{
	...
	if (IS_MSM8998_TARGET(hwversion)) {
		...
		pr_debug_once("unsupported chipset id %X\n", hwversion);
	}
}
		...
		size_t max_offset)
{
	...
	if (!name || !strlen(name)) {
		pr_debug_once("no debug name provided\n");
		return -EINVAL;
	}
		...
		uint32_t xin_id)
{
	...
	if (!reg_base) {
		pr_debug_once("error: for range %s unable to locate base %s\n",
				range_name, base_name);
		return;
	}

	if (!range_name || strlen(range_name) == 0) {
		pr_debug_once("%pS: bad range name, base_name %s, offset_start 0x%X, end 0x%X\n",
	...
	if (offset_end - offset_start < REG_DUMP_ALIGN ||
			offset_start > offset_end) {
		pr_debug_once("%pS: bad range, base_name %s, range_name %s, offset_start 0x%X, end 0x%X\n",
				__builtin_return_address(0), base_name,
